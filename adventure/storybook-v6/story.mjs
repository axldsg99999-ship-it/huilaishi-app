...
import { AutoNext } from "./assets/auto-next-v6.mjs";
...
import {SERIES_KEY,ENCOUNTERS,newSeries,readSeries,currentEncounter,startEncounter,resolveEncounter,settleEncounter,collectEncounter,retryEncounter,traitLabel} from './assets/series-v4.mjs';
import {
  PROFILE_KEY,
  DECK,
  freshProfile,
...
let series=null,seriesMode=false,resolving=false;
const autoNext = new AutoNext();
try{series=readSeries(localStorage.getItem(SERIES_KEY));}catch{}
const encounterInfo=()=>ENCOUNTERS.find(e=>e.id===battle?.enemyId)||ENCOUNTERS[1];
const encounterArt=()=>art.battle.roster[battle?.enemyId||'orchid'];
function saveSeries(){if(series)try{localStorage.setItem(SERIES_KEY,JSON.stringify(series));}catch{notify('当前浏览器无法保存三场试玩进度。');}}
function seriesPage(){
  series ||= newSeries();seriesMode=true;routeMode=false;saveSeries();
  const e=currentEncounter(series),a=art.battle.roster[e.id],stage=series.stage,won=stage==='reward',done=stage==='complete',lost=stage==='lost';
  scene.setEncounter(e.id);
  const report=series.reports.at(-1);
  let actions='',body='';
  if(won){
    body='<p>'+esc(e.memory)+'</p><div class="series-prize"><span class="seal-stamp">'+e.stamp+'</span><span><b>信印入册 · 纸币 +'+[18,24,36][series.step]+'</b><small>听懂 '+report.correct+' 句 · 精准 '+report.perfect+' 次</small></span></div>';
    actions=series.step===2?btn('series-collect:finish','拆开完整来信 '+icon('mail'),'series-primary'):
      '<p class="series-choice-note">收好信印，体力 +24。再选一份旅途礼物：</p><div class="series-gifts">'+
      btn('series-collect:cinnabar','<b>朱砂印</b><small>后续答对伤害 +3'+(series.perks.includes('cinnabar')?' · 已有，不叠加':'')+'</small>')+
      btn('series-collect:jade','<b>护身玉</b><small>后续每次受伤 −3'+(series.perks.includes('jade')?' · 已有，不叠加':'')+'</small>')+'</div>';
  }else if(done){
    body='<p class="letter-quote">“如果你也来到这里，就替我对这个世界说一句：你好。”</p><p>她留下了三次善意，你接住了三段回声。<br>阿笺把完整的信，小心地放进你的行囊。</p><div class="series-prize"><span class="seal-stamp">笺</span><span><b>三枚信印 · '+series.coins+' 旅途纸币</b><small>本系列收藏已保存，不影响正式存档</small></span></div>';
    actions=btn('series-review','回顾三次相遇','series-primary')+btn('series-reset','再走一遍','series-secondary');
  }else if(lost){
    body='<p>阿笺靠过来，用鼻尖碰碰你的手。<br>已经收到的信印还在。歇一会儿，再听一次。</p>';
    actions=btn('series-retry','恢复体力 · 重试这一场','series-primary');
  }else{
    body='<p>'+esc(e.intro)+'</p><p class="series-trait"><b>'+e.trait+'</b>'+e.tip+'</p><p class="series-instruction">听音辨义，观察动作，选招回应。<br>三招互克，不耗蓄力；阿笺每场帮你一次。</p>';
    actions=btn('series-go',(stage==='battle'?'继续这场相遇':'去见'+e.name)+' '+icon('arrow'),'series-primary');
  }
  setView('series','<header class="series-top">'+btn('home','← 码头','series-back')+'<span>风寄三封 · 战斗小系列</span>'+btn('settings',icon('gear'),'series-settings','aria-label="试玩设置"')+'</header><nav class="series-trail" aria-label="三场进度">'+
   ENCOUNTERS.map((n,i)=>'<span class="'+(series.stamps.includes(n.id)?'collected':i===series.step?'current':'')+'"><i>'+n.stamp+'</i>'+n.place+'</span>').join('')+'</nav><article class="series-copy"><small>'+(done?'三封归一':won?'相遇完成 · '+e.place:lost?'这封信，还在等你':e.chapter)+'</small><h1>'+(done?'原来，你一直在。':won?'又靠近她，一点。':lost?'先歇一会儿。':e.name)+'</h1>'+body+'<footer>'+actions+'</footer></article><img class="series-creature" src="'+a.enemyFrames[won||done?3:0].toDataURL()+'" alt="'+esc(e.name)+(won?'递上信封':'')+'"><small class="series-meta">独立试玩 · '+(series.step+1)+'/3 · 体力 '+series.hp+'/100 · 纸币 '+series.coins+'</small>');
  screen.dataset.stage=stage;
}
function seriesReview(){openFolio('风寄三封 · 相遇手记',ENCOUNTERS.map(e=>'<p><b>'+e.chapter+'</b><br>'+e.memory+'</p>').join(''),btn('close','收好这封信','main-action'));}
...
function routePowers(){
  openFolio('旅途手记','<p>已经走过：'+expedition.path.map(id=>esc(nodeById(id).name)).join(' → ')+'</p><p>强化只对本次远行生效，不改变正式角色。体力 '+expedition.hp+'/100 · 纸币 '+expedition.coins+'</p>'+
...
function cancelAudio() {
  ...
  if (voice) {
    ...
    control?.setAttribute('aria-label','听'+encounterInfo().name+'说话');
...
function home() {
  ...
  setView(
    ...
    header() +
      ...
      btn('series-open',icon('mail')+'三封来信')+
...
function begin() {
  openFolio(
    "她把信，交给了谁？",
    '<p class="eyebrow">河畔的未寄信 / 01</p><p>阿笺在台阶上嗅到熟悉的信纸味。兰螳抱着一封画着梅花的信，向你说了一句话。</p><p>先点它身边的喊话小表情听一听，再选自己的招式，点出脑海中正确的意思。</p><p><b>进攻克破势 · 破势克守势 · 守势克进攻</b></p><p>三招每轮都可直接选择。答对才有效出招；成功克制会更有力。对方招式在你选择前已锁定。答对两次，阿笺还可以帮你排除一个错误想法。' +
      (profile.calm
...
function startBattle(onRoute=false,onSeries=false) {
  routeMode=onRoute;
  seriesMode=onSeries;
  battle = onSeries ? startEncounter(series) : onRoute ? makeRouteBattle(expedition) : createBattle({
    kind: "normal",
    world: "th",
    relaxed: true,
    seed: 0,
  ...
  if(!battle)return;
  scene.setEncounter(battle.enemyId||'orchid');
  pet = onSeries ? (battle.seriesPet ||= createPetSupport()) : onRoute && battle.routePet ? battle.routePet : createPetSupport();
  ...
  setView(
    ...
    '<header class="combat-top"><div class="player-hud">' +
      ...
      '<div class="health"><b>小艾</b><div class="health-track"><i id="hero-health"></i></div><small id="hero-hp">100 / 100</small></div></div><div class="combat-title">河畔切磋<small id="round-title">听懂它，再靠近一点</small></div><div class="enemy-hud"><div class="health"><b>绣风兰螳</b><div class="health-track"><i id="enemy-health"></i></div><small id="enemy-hp"></small></div><img class="enemy-portrait" src="'+art.battle.enemyPortrait+'" alt="绣风兰螳"></div></header><div class="thoughts"></div><section class="enemy-thought" aria-label="兰螳的声音"></section><p class="combat-help" role="status" aria-live="polite"></p><div class="resolve-copy" hidden aria-live="polite"></div><p class="tactic-note"></p><p class="focus-count"></p>' +
      btn(
        "pet-help",
      ...
      STANCES.map((s, i) =>
        btn(
          ...
          'style="--tint:' +
            ...
            '" aria-pressed="false" aria-label="'+names[s]+'：'+TACTICS[s].note+'" disabled',
        ),
      ).join("") +
      ...
      '<p class="auto-next-note" hidden aria-hidden="true"></p>' +
      '</footer><section class="paused-cover" hidden><h2>风还在，等你回来。</h2><p>时间、声音和动作已暂停。</p>' +
      btn("resume", "继续这次相遇") +
      btn("leave", "回到码头") +
      '</section>'+btn('tactics','三招互克 · 看规则 ⓘ','battle-edition battle-rules','aria-label="查看招式克制和动作线索"') +
  ...
  $('.enemy-hud .health b').textContent=encounterInfo().name;
  $('.enemy-portrait').src=encounterArt().enemyPortrait;
  $('.enemy-portrait').alt=encounterInfo().name;
  if(onSeries){
    $('.combat-title').firstChild.textContent=encounterInfo().place+' · '+(series.step+1)+'/3';
    $('.battle-edition').textContent='三招互克 ⓘ · 信印 '+series.stamps.length+'/3';
    $('.battle-exit').textContent='收好进度';
    const trait=document.createElement('button');trait.className='enemy-trait';trait.dataset.action='series-trait';screen.append(trait);
  }
  renderRound();
  if(onSeries){
    if(battle.phase==='complete'){settleEncounter(series);saveSeries();seriesPage();return;}
  ...
  if(onRoute){
    ...
    $('.battle-exit').innerHTML=icon('book')+'旅途';
    if(battle.phase==='resolved')presentAnswer(battle.last,battle.last?.selectedId);
    if(battle.phase==='complete'){settleBattle(expedition);routeStage();return;}
    saveExpedition();
  }
  if(battle.phase==='prepare')scene.act("listen",1600);
}
function status(text) {
  const el = $(".combat-help");
  if (el) el.textContent = text;
  if(view==='battle')screen.dataset.notice=/无法|没能/.test(text)?'error':'';
}
function renderRound() {
  autoNext.clear();
  scene.exchange=null;scene.enemyAct(0,0);scene.setTell(battle.enemyStance);
  question = questionFor(battle.round+(seriesMode?encounterInfo().offset:routeMode?expedition.path.length*2:0));
  revealed = false;
  eliminated = null;
  ...
  screen.dataset.exchange='';
  status('点'+encounterInfo().name+'身边的喊话小表情。听完再选择自己的招式。');
}
function renderPrompt() {
  ...
  voice.setAttribute('aria-label',encounterInfo().name+'的声音');
  voice.dataset.speaking=String(guardianSpeaking);
  const calling='<span class="call-sprite" aria-hidden="true"><img class="call-rest" src="'+encounterArt().callFrames[0]+'" alt=""><img class="call-live" src="'+encounterArt().callFrames[1]+'" alt=""></span>';
  voice.innerHTML=btn('hear',calling+'<span>'+(guardianSpeaking?'说话中':battle.promptReady?'再听':'听一句')+'</span>','guardian-voice',
    'aria-pressed="'+guardianSpeaking+'" aria-label="'+(mediaBusy?'停止当前声音':'听'+encounterInfo().name+'说话')+'"')+
    btn('reveal',revealed?'收起字幕':'字幕','hint','aria-pressed="'+revealed+'" aria-label="文字辅助"');
...
function updateHealth() {
  if (!battle) return;
  if($('.enemy-trait'))$('.enemy-trait').textContent=traitLabel(battle)+' ⓘ';
...
function updateControls() {
  ...
  $('.focus-count').textContent='进攻克破势 · 破势克守势 · 守势克进攻';
  $('.tactic-note').textContent=mediaBusy?'正在聆听，计时暂停':!battle.promptReady?'听一句，再选招式':battle.stance?TACTICS[battle.stance].note:'观察动作 · 三种招式都可选';
}
async function play(text, lang, { prompt = false, dialogue = false } = {}) {
  ...
  const token = mediaEpoch,
    ...
    activeView = view;
  mediaBusy = true;
  guardianSpeaking = prompt && activeView === 'battle' && !dialogue;
...
function answer(id) {
  ...
  if(routeMode)saveExpedition();
  if(seriesMode)saveSeries();
  presentAnswer(result,id);
}
function presentAnswer(result,id) {
  if(!result)return;
  autoNext.clear();
  const answeredBattle = battle;
  resolving=true;
  scene.setTell(null);screen.dataset.hit=result.correct?'enemy':'hero';
  screen.dataset.exchange='windup';
  $('#hero-health').style.width=(result.hpBefore??battle.hp+result.taken)+'%';
  $('#enemy-health').style.width=((result.enemyHpBefore??battle.enemyHp+result.dealt)/battle.enemyMax*100)+'%';
  $('#hero-hp').textContent=(result.hpBefore??battle.hp+result.taken)+' / 100';
  $('#enemy-hp').textContent=(result.enemyHpBefore??battle.enemyHp+result.dealt)+' / '+battle.enemyMax;
  updateControls();
  ...
  feedback.innerHTML =
    "<b>" +
    (result.correct
      ? result.tactical==='counter'?{attack:'截住破势',break:'破势成功',guard:'守势反击'}[result.stance]:result.tactical==='exposed'?'词义答对 · 招式受制':result.tactical==='even'?'同招相接':'听懂了'
      : "再听一次") +
    '</b><small>'+esc([result.correct&&names[result.enemyStance]?(result.tactical==='even'?'双方同招':result.tactical==='counter'?names[result.stance]+'克制'+names[result.enemyStance]:names[result.enemyStance]+'克制'+names[result.stance]):'',...(result.notes||[])].filter(Boolean).join(' · '))+'</small><p>' +
    esc(question.unit.th) +
    " · " +
    esc(question.unit.zh) +
    "</p>";
  scene.exchangeAnswer(result,{
    impact:()=>{
      if(view!=='battle'||battle!==answeredBattle)return;
      screen.dataset.exchange='impact';updateHealth();feedback.hidden=false;
      if(result.correct){scene.enemyAct(battle.outcome==='win'?14:8,900);scene.petAct(3,1200);if(result.taken)scene.act('hit',650);}
      else scene.act(result.stance==='guard'?'guard':'hit',800);
      if(profile.sound)playFeedback(result.correct,result);
    },
    finish:()=>{
      if(view!=='battle'||battle!==answeredBattle)return;
      resolving=false;screen.dataset.exchange='settled';
      autoNext.arm(battle);
      $('.auto-next-note').hidden=false;
      if(battle.outcome==='win'){scene.act('victory',2200);scene.enemyAct(14,2200);scene.burst('reward',.74,.55);}
    },
  });
  status(
    result.correct
  ...
  note.textContent =
    battle.outcome === "win"
      ...
      : battle.outcome === "lose"
        ? "即将回到台阶休息"
        : result.correct ? "下一句，自动接上" : "记住这句，稍后自动继续";
}
function next() {
  if(resolving)return;
...
function tick(dt) {
  if(seriesMode&&performance.now()-routeSaveAt>1500){routeSaveAt=performance.now();saveSeries();}
  if(routeMode&&performance.now()-routeSaveAt>1500){routeSaveAt=performance.now();saveExpedition();}
  if (autoNext.tick(dt, battle, view !== 'battle' || resolving || mediaBusy || paused || hiddenPause || folio.open || innerHeight > innerWidth)) {
    next();
    return;
  }
  if (
...
function petRoom() {
  openFolio(
    ...
    '<div class="pet-folio"><img class="pet-portrait" src="' +
      ...
      '" alt="阿笺，一只带着信袋的小狗"><div><p class="eyebrow">随行伙伴 / 小狗阿笺</p><h3>今天也陪你，一点点学会。</h3><p>亲密度 ' +
      profile.closeness +
      " · " +
...
function playFeedback(correct,record={}) {
  ...
  try {
    ...
    const paper=audioContext.createBufferSource(),filter=audioContext.createBiquadFilter(),paperGain=audioContext.createGain();
    paper.buffer=noise;filter.type='bandpass';filter.frequency.value=record.stance==='guard'?740:1900;paperGain.gain.value=.085;
    paper.connect(filter);filter.connect(paperGain);paperGain.connect(audioContext.destination);paper.start(t);
    const notes=correct?(record.sealBroken||record.armorBroken?[392,784,1175]:record.timing==='perfect'?[587,880,1175]:[196,587]):[130,196];
    for (let i = 0; i < notes.length; i++) {
      const osc = audioContext.createOscillator(),
        gain = audioContext.createGain();
      osc.type = i===0?'triangle':'sine';
      osc.frequency.setValueAtTime(notes[i],t+i*.07);
      if(i===0)osc.frequency.exponentialRampToValueAtTime(notes[i]*.55,t+.14);
      gain.gain.setValueAtTime(0, t + i * 0.07);
      gain.gain.linearRampToValueAtTime(i===0?.065:.025, t + i * 0.07 + 0.015);
...
async function action(value) {
  ...
  switch (key) {
    case 'tactics':openFolio('三招互克 · 不耗蓄力','<p><b>进攻克破势 → 破势克守势 → 守势克进攻</b></p><p>同招正常交锋；答对且克制伤害更高。答对但受制仍有伤害，也可能受到反击。答错不能靠猜中招式造成伤害。</p><p>观察身体动作：<br>前倾逼近、脚后有疾风：进攻。<br>侧身绕步、纸叶绕行：破势。<br>收身护住、纸扇护在身前：守势。</p><p>对方每轮在你选择前锁定招式，切换招式、重听或刷新都不会让它临时改招。词句语音用于辨义，身体动作用于判断招式。</p>',btn('close','知道了，继续相遇','main-action'));break;
    case 'series-open': seriesPage();break;
    case 'series-go': if(['intro','battle'].includes(series?.stage))startBattle(false,true);break;
    case 'series-collect': if(collectEncounter(series,arg)){saveSeries();seriesPage();}break;
    case 'series-retry': if(retryEncounter(series)){saveSeries();seriesPage();}break;
    case 'series-review':seriesReview();break;
    case 'series-reset':openFolio('重新走这三封信？','<p>只重置本次小系列的信印、增益和纸币。正式存档不会变化。</p>',btn('close','先保留')+btn('series-confirm-reset','重新开始','main-action'));break;
    case 'series-confirm-reset':series=newSeries();seriesPage();break;
    case 'series-trait':openFolio(encounterInfo().trait,'<p>'+esc(encounterInfo().tip)+'</p><p>这是当前机制，不是对方下一招的预告。</p>',btn('close','记住了','main-action'));break;
    case "home":
      home();
      break;
    case "begin":
    ...
    case "resume":
      ...
      break;
    case "leave":
      if(seriesMode){saveSeries();seriesPage();}
      else if(routeMode){saveExpedition();routeSelected=expedition.active;routeMap();}
      else home();
      break;
    case 'route-open':
    ...
    case 'route-go':
      if(view==='map'&&enterNode(expedition,arg)){
        saveExpedition();
        const cover=$('.journey-departure');cover.hidden=false;cover.textContent='前往 · '+nodeById(arg).name;
        if(motion())await new Promise(resolve=>setTimeout(resolve,650));
        routeStage();
      }
      break;
...
try {
  ...
  if(new URLSearchParams(location.search).has('series'))seriesPage();
  else if(new URLSearchParams(location.search).has('map')) routeMap();
