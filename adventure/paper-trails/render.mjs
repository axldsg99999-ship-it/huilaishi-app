import { sprite, cover, shadow, seal } from './art.mjs?v=feel1';
...
export function paintWorld(c, art, state, fx) {
 ...
 const guardian=(name,x,y,h,hit=0,speaking=false,alpha=1)=>{
  ...
  if(name==='bear')key=celebrating||r?.catchUntil>r?.clock?'bear-cheer':r?.phase==='catching'?'bear-cast':name;
  shadow(c,x,y,h*.25,alpha*.15);sprite(c,art[key]||art[name],x+(!reduced&&hit>0?Math.sin(hit*18)*Math.exp(-hit*2)*22:0),y+breath,h,{angle:wobble,sx:1+wobble*.7,sy:1-wobble*.5,alpha,flip:name==='elephant'&&key===name});
