...
function crop(im, box, isolate = false) {
  const [x,y,w,h] = box;
...
export async function loadBattleArt() {
  const [backdrop,hero,enemy,kit]=await Promise.all([
    image(file('battle-stage-v2.png')),image(file('xiaoai-battle-v2.png')),
  ...
  return {backdrop,heroFrames,enemyFrames,paper,
