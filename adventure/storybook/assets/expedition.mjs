...
import {createBattle, BEATS} from '../runtime/lab/engine.mjs';
...
export const POWERS=Object.freeze([
  ...
  {id:'orchid',name:'兰花结',icon:'break',color:'#735786',text:'答对且姿态克制对手时，每级额外造成 3 点伤害。'},
...
export function applyRouteAnswer(run,b,result){
  ...
  if(result.correct){
    ...
    if(BEATS[result.stance]===result.enemyStance)extra+=3*(run.powers.orchid||0);
  ...
  // Keep the elite's one telegraphed finishing window intact.
  if(b.kind==='elite'&&!b.heroUsed&&!b.hero)extra=Math.min(extra,Math.max(0,b.enemyHp-18));
  extra=Math.min(extra,b.enemyHp);
  // Mitigate before HP clamping. Otherwise a low-HP guard could become immortal.
  const beforeHp=b.hp+result.taken;
  const rawTaken=result.correct?(result.tactical==='exposed'?(result.timing==='perfect'?2:7):0):(result.stance==='guard'?7:12);
