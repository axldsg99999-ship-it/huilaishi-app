...
export class PierScene {
    ...
    if (combat || (reward && h > 500)) {
      ...
      if(combat) {
        ...
        this.battleActor(encounter.enemyFrames,frame,ex,ground,h*(this.enemyId==='elephant'?.40:.435),{
          dx:action.enemyDx*w+action.shake,lift:breathe*.7,lean:this.motion&&this.enemyPose===8?.055:0,
        });
      } else {
  ...
  drawExchange(){
    ...
    if(r.armorBroken||r.sealBroken||r.timing==='perfect'&&r.correct){
      ...
      c.font='bold '+(s*.7)+'px Wenkai';c.fillStyle='#ffe7b8';c.fillText(r.armorBroken||r.sealBroken?'破':'合',0,0);c.restore();
    }
    c.restore();
  }
  drawEffects() {
