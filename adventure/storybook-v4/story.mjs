...
function seriesPage(){
  ...
  if(won){
    ...
    body='<p>'+esc(e.intro)+'</p><p class="series-trait"><b>'+e.trait+'</b>'+e.tip+'</p><p class="series-instruction">点它的喊话表情听音 → 选招式 → 点自己的想法。<br>答对积攒蓄力，满 2 点可破势；阿笺每场帮你一次。</p>';
...
function begin() {
  openFolio(
    "她把信，交给了谁？",
    '<p class="eyebrow">河畔的未寄信 / 01</p><p>阿笺在台阶上嗅到熟悉的信纸味。兰螳抱着一封画着梅花的信，向你说了一句话。</p><p>先点它身边的喊话小表情听一听，再选自己的招式，点出脑海中正确的意思。</p><p><b>进攻伤害高 · 守势更稳妥 · 蓄力满 2 可破势</b></p><p>进攻或守势答对可蓄力，破势消耗 2 点蓄力。答对两次，阿笺还可以帮你排除一个错误想法。' +
      (profile.calm
...
function startBattle(onRoute=false,onSeries=false) {
  ...
  if(onSeries){
    ...
    $('.battle-edition').textContent='风寄三封 · 信印 '+series.stamps.length+'/3';
  ...
  if(battle.phase==='prepare'){scene.act("listen",1600);scene.enemyAct(14,1600);}
}
...
function updateControls() {
  ...
  $('.focus-count').textContent='蓄力 '+ '●'.repeat(battle.focus)+'○'.repeat(3-battle.focus)+' · 破势需 2';
  $('.tactic-note').textContent=mediaBusy?'正在聆听，计时暂停':!battle.promptReady?'听一句，再选招式':battle.stance?TACTICS[battle.stance].note:'进攻抢伤害 · 守势更稳妥';
}
...
function presentAnswer(result,id) {
  ...
  feedback.innerHTML =
    ...
    (result.correct
      ? result.timing === "perfect"
        ? "心意相通"
        : result.tactical === "interrupt"
          ? "漂亮破招"
          : "听懂了"
      : "再听一次") +
    '</b><small>'+esc((result.notes||[]).join(' · '))+'</small><p>' +
  ...
  scene.exchangeAnswer(result,{
    impact:()=>{
      ...
      if(result.correct){scene.enemyAct(battle.outcome==='win'?14:8,900);scene.petAct(3,1200);}
