import {createBattle,resolveAnswer} from './combat-v3.mjs';
...
export const ENCOUNTERS=Object.freeze([
 {id:'elephant',name:'晨市小象',chapter:'第一封 · 果篮里的问候',place:'晨市',hp:64,offset:0,
  trait:'藤篮护甲',tip:'藤篮挡住前 12 点伤害。破势能直接拆开护甲。',
 ...
 {id:'kite',name:'钟楼守信兽',chapter:'第三封 · 纸翼归来',place:'钟楼',hp:108,offset:5,
  trait:'两重纸封',tip:'纸封每层减伤 4。精准或破势拆一层，过半血后答错更危险。',
...
export function startEncounter(s){
 ...
 const e=currentEncounter(s),b=createBattle({kind:e.id==='kite'?'elite':'normal',relaxed:true});
...
export function resolveEncounter(b,q,id){
 ...
 if(r.correct){
  ...
  if(b.enemyId==='elephant'&&b.armor){
   block=r.stance==='break'?0:Math.min(b.armor,damage);
   b.armor=r.stance==='break'?0:b.armor-block;
  ...
  if(b.enemyId==='kite'){
   if(b.seals&&(r.stance==='break'||r.timing==='perfect')){b.seals--;r.sealBroken=true;r.notes.push('纸封散开');}
   block=b.seals*4;
 ...
 try{
  ...
  if(['battle','reward','lost','complete'].includes(s.stage)){
   const b=s.battle;if(!b||b.enemyId!==currentEncounter(s).id||!['prepare','active','resolved','complete'].includes(b.phase)||!Array.isArray(b.records)||
    ...
    !Number.isInteger(b.focus)||b.focus<0||b.focus>3||!Number.isInteger(b.round)||b.round<0||!Number.isInteger(b.combo)||b.combo<0||
