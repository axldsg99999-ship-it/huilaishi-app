import {stopCreatureAudio} from './assets/creature-audio/player.mjs';
import {blockSceneSound} from './scene-audio.mjs';
...
async function playSpoken(
  ...
  if (clip) {
    return new Promise((resolve) => {
      const a = new Audio(new URL(clip.startsWith('@/') ? './'+clip.slice(2) : '../'+clip, new URL('../../voice.mjs', import.meta.url)).href);
      activeAudio = a;
      a.playbackRate = rate;
      let settled = false;
      const done = (ok) => {
