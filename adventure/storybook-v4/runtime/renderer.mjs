import { ASSET, MONSTERS } from "./content.mjs";
import {companionArt} from './core.mjs';
import {combatTiming,combatMotion} from './battle-presentation.mjs';
import {LivingScene} from './living-scenes.mjs';
...
// A lightweight costume layer keeps the authored action atlases alive while
// giving Xiao Ai the same visual identity as the "Qingzhu Letter" concept.
// It is deliberately drawn as cloth, trim and a sewn tag rather than as a
// rectangular UI overlay, so the design follows the actor through every pose.
function drawHanfuBack(ctx,w,h,x,ground,height,facing,rotation,offset,elapsed){
  const sway=Math.sin(elapsed/520)*height*.008;
  ctx.save();
  ctx.translate(x*w+offset,ground*h);
  ctx.scale(facing,1);
  ctx.rotate(rotation*.55);
  ctx.globalAlpha=.84;
  ctx.fillStyle='#234f52';
  ctx.strokeStyle='#a9b59a';
  ctx.lineWidth=Math.max(.8,height*.006);
  ctx.beginPath();
  ctx.moveTo(-height*.13,-height*.78);
  ctx.quadraticCurveTo(-height*.26,-height*.68,-height*.25,-height*.46);
  ctx.quadraticCurveTo(-height*.28,-height*.31,-height*.19,-height*.23+sway);
  ctx.lineTo(-height*.07,-height*.31);
  ctx.lineTo(height*.08,-height*.29);
  ctx.quadraticCurveTo(height*.24,-height*.36,height*.22,-height*.61);
  ctx.quadraticCurveTo(height*.19,-height*.76,height*.10,-height*.81);
  ctx.closePath();
  ctx.fill();ctx.stroke();
  // Bamboo shadow weave: two quiet lines are enough at phone scale.
  ctx.globalAlpha=.36;ctx.strokeStyle='#c2c79c';ctx.lineWidth=Math.max(.65,height*.004);
  for(let i=0;i<3;i++){
    const xx=-height*(.18-i*.055);
    ctx.beginPath();ctx.moveTo(xx,-height*.68);ctx.quadraticCurveTo(xx-height*.025,-height*.48,xx+height*.015,-height*.30);ctx.stroke();
  }
  ctx.restore();
}
function drawHanfuFront(ctx,w,h,x,ground,height,facing,rotation,offset,elapsed,action){
  const flutter=(action==='walk'||action==='strike'||action==='dodge')?Math.sin(elapsed/150)*height*.012:0;
  ctx.save();
  ctx.translate(x*w+offset,ground*h);
  ctx.scale(facing,1);
  ctx.rotate(rotation);
  // Open robe fronts sit over the original animated torso and conceal the
  // modern colour blocks without covering the hands, face or weapon.
  ctx.globalAlpha=.93;
  ctx.fillStyle='#e9e0ca';
  ctx.strokeStyle='#f8f0db';
  ctx.lineWidth=Math.max(1,height*.007);
  ctx.beginPath();
  ctx.moveTo(-height*.082,-height*.755);
  ctx.lineTo(-height*.012,-height*.655);
  ctx.lineTo(-height*.045,-height*.315);
  ctx.lineTo(-height*.155,-height*.31+flutter);
  ctx.lineTo(-height*.12,-height*.54);
  ctx.closePath();
  ctx.fill();ctx.stroke();
  ctx.beginPath();
  ctx.moveTo(height*.082,-height*.755);
  ...
  ctx.closePath();
  ctx.fill();ctx.stroke();
  // Cinnabar lining and the red woven waist sash.
  ctx.globalAlpha=.96;ctx.strokeStyle='#a64b3e';ctx.lineWidth=Math.max(1,height*.011);
  ctx.beginPath();ctx.moveTo(-height*.082,-height*.755);ctx.lineTo(-height*.012,-height*.655);ctx.lineTo(-height*.045,-height*.32);ctx.stroke();
  ctx.beginPath();ctx.moveTo(height*.082,-height*.755);ctx.lineTo(height*.012,-height*.655);ctx.lineTo(height*.045,-height*.32);ctx.stroke();
  ctx.fillStyle='#a84e40';ctx.beginPath();
  ctx.moveTo(-height*.165,-height*.395);ctx.lineTo(height*.165,-height*.405);ctx.lineTo(height*.17,-height*.345);ctx.lineTo(-height*.16,-height*.335);ctx.closePath();ctx.fill();
  ctx.globalAlpha=.36;ctx.strokeStyle='#f3d39b';ctx.lineWidth=Math.max(.7,height*.004);
  for(let i=-2;i<4;i++){const xx=height*(i*.055-.12);ctx.beginPath();ctx.moveTo(xx,-height*.398);ctx.lineTo(xx+height*.018,-height*.345);ctx.stroke();}
  // Jade seal and the small sewn 艾 tag are the identity anchors.
  ctx.globalAlpha=1;ctx.fillStyle='#719b8d';ctx.strokeStyle='#e5d2a2';ctx.lineWidth=Math.max(.8,height*.005);
  ctx.beginPath();ctx.roundRect?.(-height*.026,-height*.305,height*.052,height*.064,height*.009);if(!ctx.roundRect){ctx.rect(-height*.026,-height*.305,height*.052,height*.064);}ctx.fill();ctx.stroke();
  ctx.strokeStyle='#d7c99c';ctx.lineWidth=Math.max(.7,height*.004);ctx.beginPath();ctx.moveTo(0,-height*.337);ctx.lineTo(0,-height*.305);ctx.stroke();
  ctx.fillStyle='#f4ebd3';ctx.strokeStyle='#b8845f';ctx.lineWidth=Math.max(.6,height*.003);
  ctx.beginPath();ctx.rect(height*.07,-height*.62,height*.055,height*.065);ctx.fill();ctx.stroke();
  ctx.fillStyle='#6c3e35';ctx.font='600 '+Math.max(7,height*.034)+'px serif';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText('艾',height*.0975,-height*.5875);
  // A short tassel gives the sash a living, hand-made finish in motion.
  ctx.strokeStyle='#aa4c3e';ctx.lineWidth=Math.max(1,height*.006);ctx.beginPath();ctx.moveTo(height*.14,-height*.345);ctx.lineTo(height*.155,-height*.235+flutter);ctx.stroke();
  ctx.fillStyle='#d0a151';ctx.beginPath();ctx.arc(height*.155,-height*.225+flutter,height*.012,0,Math.PI*2);ctx.fill();
  ctx.restore();
}
...
export class Stage {
  ...
  async equip(name) {
    ...
    this.canvas.dataset.costume = this.hanfu ? (this.options.hanfuStyle || 'hanfu') : 'default';
    try {
  ...
  tick(now) {
    ...
    if (!document.hidden) {
      ...
      if(this.hanfu&&heroArt)drawHanfuBack(ctx,this.w,this.h,heroX,ground,hs,this.heroFacing,heroIdle.lean+hrot,hoff,now);
      this.actor(
        heroArt,
        drawFrame,
        heroX,
      ...
      if(this.hanfu&&heroArt)drawHanfuFront(ctx,this.w,this.h,heroX,ground,hs,this.heroFacing,heroIdle.lean+hrot,hoff,now,action);
      ...
      if(this.options.namePin&&!this.hanfu&&emotionFrame===null&&interactionFrame===null&&action==='idle'){
