(() => {
  ...
  const COURSES = {
    "zh-th": {
      ...
      correct: "听对了",
      wrong: "再听一次",
      review: "课程给出实用 IPA 与带调罗马音。中文近音只帮记忆，不能代替泰语原音；审核边界见「我的 > 内容与声音说明」。",
      lessons: [
        {
  ...
  const canUseFullCourse = () => globalThis.SawadeekaCommerce?.canAccess?.("pronunciation-full") !== false;
  function requestFullCourse() {
    globalThis.SawadeekaCommerce?.openPaywall?.("pronunciation-full");
  }
  ...
  function playBundledAudio(key, button, playbackRate) {
    ...
    stopAudio();
    const feedback = state.root?.querySelector(".pc-audio-feedback");
    ...
    audio.preload = "auto";
    audio.volume = .96;
    audio.playbackRate = Math.max(.8, Math.min(1, Number(playbackRate) || 1));
    ...
    const fail = error => {
      if (runId !== state.audioRun || state.audio !== audio) return;
      clear();
      ...
      const localRetry = state.direction === "th-zh"
        ? "ยังเล่นเสียงบทนี้ไม่ได้ ลองอีกครั้งหรือเปิดบทเรียนใหม่"
        : "本课音频暂时无法播放，请重试或重新打开课程。";
      if (feedback) feedback.textContent = ["audio-unavailable", "decode-error"].includes(reason)
        ? localRetry : window.HUILAISHI_SPEECH?.safety?.playbackErrorText?.(reason, state.direction === "th-zh") || localRetry;
    };
  ...
  function lessonMarkup(lesson, course) {
    const exampleRows = lesson.examples.map((item, index) => `
      <button class="pc-example" type="button" data-pc-action="speak" data-audio-key="${state.direction}__${lesson.id}__example-${index}" data-audio-rate="1" data-text="${escapeHtml(item.text)}" data-rate="1" aria-label="${escapeHtml(course.listen)} ${escapeHtml(item.text)}">
    ...
    const chunks = lesson.model.chunks.map((chunk, index) => `
      <button type="button" class="pc-chunk" data-pc-action="speak" data-audio-key="${state.direction}__${lesson.id}__chunk-${index}" data-audio-rate="1" data-text="${escapeHtml(chunk)}" data-rate=".8">
    ...
    const mnemonic = state.direction === "zh-th" && lesson.model.mnemonic
      ? `<div class="pc-mnemonic"><span>发音助记 · 以原音为准</span><b>${escapeHtml(lesson.model.mnemonic)}</b></div>` : "";
    ...
    return `
      <article class="pc-lesson" aria-labelledby="pc-lesson-title">
        ...
        <div class="pc-rule-grid">
          ...
          <aside><span>${state.direction === "zh-th" ? "发音要点" : "หลักการออกเสียง"}</span>${escapeHtml(lesson.rule)}</aside>
        </div>
        <section class="pc-section" aria-labelledby="pc-examples-title">
          <div class="pc-section-title"><span id="pc-examples-title">${escapeHtml(course.examples)}</span><small>IPA · ${state.direction === "zh-th" ? "罗马音" : "พินอิน"}</small></div>
          <div class="pc-examples">${exampleRows}</div>
        </section>
        <section class="pc-model">
          <div class="pc-model-top"><span>${state.direction === "zh-th" ? "整句示范" : "ประโยคตัวอย่าง"}</span><small>${escapeHtml(lesson.model.translation)}</small></div>
          ...
          <div class="pc-listen-row">
            <button type="button" data-pc-action="speak" data-audio-key="${state.direction}__${lesson.id}__model" data-audio-rate="1" data-text="${escapeHtml(lesson.model.text)}" data-rate="1"><i aria-hidden="true">▶</i>${escapeHtml(course.listen)}</button>
            <button type="button" data-pc-action="speak" data-audio-key="${state.direction}__${lesson.id}__model" data-audio-rate=".8" data-text="${escapeHtml(lesson.model.text)}" data-rate=".8"><i aria-hidden="true">◌</i>${escapeHtml(course.slow)}</button>
          </div>
          <p class="pc-audio-feedback" role="status" aria-live="polite"></p>
        ...
        <section class="pc-quiz" aria-labelledby="pc-quiz-title">
          <div class="pc-section-title"><span id="pc-quiz-title">${escapeHtml(course.practice)}</span><small>${state.direction === "zh-th" ? "听辨练习" : "ฝึกแยกเสียง"}</small></div>
  ...
  function shellMarkup() {
    ...
    const rail = course.lessons.map((item, index) => `
      <button type="button" data-pc-action="lesson" data-index="${index}" ${index > 0 ? 'data-premium-feature="pronunciation-full"' : ""} aria-label="${escapeHtml(course.lesson)} ${item.no} · ${escapeHtml(item.title)}" title="${escapeHtml(item.title)}" class="${index === state.index ? "active" : ""}" aria-current="${index === state.index ? "step" : "false"}">
  ...
  function handleClick(event) {
    ...
    if (action === "speak") {
      ...
      if (!playBundledAudio(button.dataset.audioKey, button, button.dataset.audioRate)) {
        speak(text, targetLang(), rate);
      }
      return;
    }
    if (action === "lesson") {
      const nextIndex = Math.max(0, Math.min(currentCourse().lessons.length - 1, Number(button.dataset.index)));
      if (nextIndex > 0 && !canUseFullCourse()) { requestFullCourse(); return; }
      state.index = nextIndex;
      render({ focusHeading: true }); announceNavigation(); return;
    }
    if (action === "previous" || action === "next") {
      if (action === "next" && state.index === 0 && !canUseFullCourse()) { requestFullCourse(); return; }
  ...
  function handleEntitlementChange() {
    if (state.index <= 0 || canUseFullCourse()) return;
    state.index = 0;
    render();
  }
  ...
  function init(options = {}) {
    ...
    document.addEventListener("sawadeeka:entitlement-change", handleEntitlementChange);
  ...
  function destroy() {
    ...
    if (state.listenersBound) document.removeEventListener("sawadeeka:entitlement-change", handleEntitlementChange);
    document.documentElement.classList.remove("pc-modal-open");
