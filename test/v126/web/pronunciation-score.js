(() => {
  ...
  const ui = () => globalThis.HUILAISHI_NATIVE_ANDROID
    ? { ...copy[direction()], noRecognition: direction() === "zh-th"
      ? "安卓测试版正在排查语音接口兼容问题，自动识别与转写评分暂未开放；可以录音回听，不影响示范音播放。"
      : "แอป Android รุ่นทดสอบกำลังตรวจสอบระบบเสียง จึงยังไม่เปิดการรู้จำและคะแนนถอดเสียง แต่ยังบันทึกเสียงฟังย้อนหลังและฟังเสียงตัวอย่างได้" }
    : copy[direction()];
  ...
  async function recognizeTarget(options = {}) {
    ...
    if (globalThis.HUILAISHI_NATIVE_ANDROID) return empty("none");
