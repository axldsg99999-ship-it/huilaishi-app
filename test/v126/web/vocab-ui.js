(function () {
  ...
  function finishPendingGamePractice() {
    ...
    if (!entry || !/^rogue:[a-zA-Z0-9:-]{1,150}$/.test(entry.attemptId || "") || !allWords().some(w => w.id === entry.id)
  ...
  function recordGamePractice(id, correct, attemptId) {
    ...
    if (!word || typeof correct !== "boolean" || !/^rogue:[a-zA-Z0-9:-]{1,150}$/.test(attemptId || "")) return null;
    ...
    writeJson(key("game-practice-pending"), {
      entry: { id, level: Number(word.level), correct, source: "rogue", attemptId, at: now },
