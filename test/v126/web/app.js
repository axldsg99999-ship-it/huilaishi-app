...
const FEATURE_BUNDLES = Object.freeze({
  ...
  games: Object.freeze({
    styles: Object.freeze(["arcade.css", "combat-stage.css", "campus-stage.css", "campus-adventure.css", "world-atlas.css", "rogue-expedition.css", "expedition-polish.css"]),
...
function showWelcome() {
  ...
  text("#welcome-step-learn", zh ? "学 3 个词" : "เรียน 3 คำ");
  text("#welcome-step-play", zh ? "打第一场" : "สู้ครั้งแรก");
  text("#welcome-step-relic", zh ? "选一件强化" : "เลือกพลังเสริม");
...
async function startWelcomeAdventure() {
  ...
  try {
    ...
    skipWelcome();
    navigate("battle");
    await window.ArcadeUI?.openRogue?.({ guided: true, direct: true, onGuideDone: returnToHub });
...
function applyDirection(direction, persist = true) {
  ...
  const compactNav = isChineseUi
    ? { home: "营地", live: "练习", battle: "对战", profile: "我的" }
    : { home: "แคมป์", live: "ฝึก", battle: "ดวล", profile: "ฉัน" };
  ...
  $('.logo-button').setAttribute('aria-label', isChineseUi ? '返回营地' : 'กลับแคมป์');
  $('.logo-button').dataset.speakText = isChineseUi ? '返回营地' : 'กลับแคมป์';
