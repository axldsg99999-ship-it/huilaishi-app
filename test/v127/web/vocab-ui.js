(function () {
  ...
  const COPY = {
    zh: {
      ...
      search: "搜泰文、中文、拼音、罗马音或中文近音", category: "场景", states: ["全部", "未学", "待复习", "收藏", "错词"], shuffle: "换一组", words: "张训练卡", more: "再看 12 张",
      emptyTitle: "这一组暂时没有词", emptyCopy: "换个等级、筛选条件或搜索词试试。", audio: "原速 1×", slowAudio: "慢听 0.8×", star: "收藏", starred: "已收藏", known: "已掌握", markKnown: "标为掌握",
    ...
    th: {
      ...
      search: "ค้นหาจีน ไทย พินอิน หรือคำอ่าน", category: "สถานการณ์", states: ["ทั้งหมด", "ยังไม่เรียน", "ถึงเวลาทบทวน", "รายการโปรด", "คำที่พลาด"], shuffle: "สลับชุด", words: "การ์ดฝึก", more: "ดูเพิ่ม 12 การ์ด",
      emptyTitle: "ยังไม่มีคำในชุดนี้", emptyCopy: "ลองเปลี่ยนระดับ ตัวกรอง หรือคำค้นหา", audio: "ปกติ 1×", slowAudio: "ช้า 0.8×", star: "บันทึก", starred: "บันทึกแล้ว", known: "จำได้แล้ว", markKnown: "ทำเครื่องหมายว่าจำได้",
  ...
  function renderLevels() {
    ...
    q("#vocab-level-grid").innerHTML = Array.from({ length: 6 }, (_, index) => {
      ...
      return `<button class="vocab-level-card ${level === activeLevel ? "active" : ""}" data-vocab-level="${level}" data-level="${level}" ${level > 1 ? 'data-premium-feature="vocab-l2-l6"' : ""} style="--level-color:${meta.color};--level-progress:${progress}%"><span>L${level}</span><b>${esc(meta.name)}</b><small>${words.length || 0} ${esc(copy().words)}</small><i></i></button>`;
    }).join("");
  }
  ...
  function finishPendingGamePractice() {
    ...
    if (!entry || !/^(rogue|world):[a-zA-Z0-9:-]{1,150}$/.test(entry.attemptId || "") || !allWords().some(w => w.id === entry.id)
  ...
  function recordGamePractice(id, correct, attemptId) {
    ...
    if (!word || typeof correct !== "boolean" || !/^(rogue|world):[a-zA-Z0-9:-]{1,150}$/.test(attemptId || "")) return null;
    ...
    const box = correct ? Math.min(5, Number(current.box || 0) + (promote ? 1 : 0)) : 1;
    writeJson(key("game-practice-pending"), {
      entry: { id, level: Number(word.level), correct, source: attemptId.startsWith("world:") ? "world" : "rogue", attemptId, at: now },
      srs: { box, seen: Number(current.seen || 0) + 1, due: correct ? now + SRS_DAYS[box] * DAY : now, lastGamePromotion: promote ? now : Number(current.lastGamePromotion || 0) },
  ...
  function openReview(options = {}) {
    ...
    const allowedStates = new Set(["all", "new", "due", "star", "wrong"]);
    activeLevel = level;
    activeCategory = "all";
