...
(function (root) {
  ...
  function header(s) {
    ...
    return `<header class="rx-hud">${r ? `<span><small>${s.dir==="zh-th"?"当前关卡":"ด่านปัจจุบัน"}</small><b>${r.floor+1} / 9</b></span><span><small>${c.hp}</small><b>${r.hp}/${r.maxHp}</b>${meter(r.hp,r.maxHp,c.hp)}</span><span><small>${c.coins}</small><b>${r.coins}</b></span>` : `<span><small>${c.clears}</small><b>${s.state.meta.wins}</b></span>`}<span class="rx-save ${s.durable ? "" : "rx-warning"}" role="status">${s.durable ? c.saved : c.volatile}</span></header>`;
  }
...
return `<section class="rx-cover" style="--rx-scene:url('${esc(bg(s))}')"><div class="rx-cover-copy"><span class="rx-kicker">${esc(name(s,WORLD?.profile(s.profile)))||c.sub}</span><h3>${c.title}</h3><p>${c.world}</p><small>${c.newNote}</small>${button(ongoing?"resume":"start",ongoing?c.resume:c.start,"",true)}${ongoing?button("abandon",c.abandon):""}</div><img src="${esc(s.hero.art)}" alt="" class="rx-cover-hero" decoding="async"></section>${!ongoing?`<details class="rx-route-settings" ${s.routeSettingsOpen?'open':''}><summary>${c.changeRegion}</summary>${destination(s)}</details>`:''}${P.journal(s)}${P.wardrobe(s)}${P.bestiary(s)}${growth(s)}${s.rejected?`<p class="rx-warning" role="status">${c.damaged}</p>`:""}`;
  ...
  function destination(s){
    ...
    return `<section class="rx-destination"><label for="rx-region">${c.destination}</label><select id="rx-region" data-rx-region>${WORLD.profiles().filter(p=>!p.direction||p.direction===s.dir).map(p=>`<option value="${p.id}" ${p.id===s.profile?'selected':''}>${esc(name(s,p))}${s.canEnter?.(p)===false?` · ${c.premium}`:''}</option>`).join('')}</select><div class="rx-kind">${button('kind',c.vocabulary,`data-value="words" aria-pressed="${s.kind==='words'}"`)}${button('kind',c.sentences,`data-value="sentences" aria-pressed="${s.kind==='sentences'}" ${p.level<3?'disabled':''}`)}</div><p>${c.content} · ${data.length} ${s.kind==='sentences'?c.sentenceCount:c.wordPairs} · ${p.monsterIds.length} ${c.roster}</p><small>${metrics.regions} ${s.dir==='zh-th'?'区域':'พื้นที่'} · ${metrics.monsterConfigurations} ${c.roster}</small>${s.kind==='sentences'?`<p class="rx-warning">${c.sentenceNote}</p>`:''}</section>`;
  }
  function growth(s){const c=text(s),m=s.state.meta;return `<section class="rx-growth"><b>${c.level} ${M.level(m)} · ${c.clears} ${m.wins} · ${c.collection} ${m.discoveries.length}</b><p>${m.xp} ${c.xp} · ${s.dir==="zh-th"?"战斗经验提升角色能力；旅途道具只在本局生效。":"ประสบการณ์ช่วยเพิ่มพลังตัวละคร ไอเทมมีผลเฉพาะรอบนี้"}</p></section>`;}
  ...
  function result(s) {
    const c=text(s),r=s.state.run;
    return `<section class="rx-paper rx-summary"><span class="rx-kicker">${r.phase==="won"?"CHAPTER CLEAR":"TO BE CONTINUED"}</span><h3 tabindex="-1" data-rx-focus>${r.phase==="won"?c.won:c.lost}</h3><p>${c.resultNote}</p><div class="rx-summary-grid"><span><b>${r.correct}/${r.answers}</b>${c.practice}</span><span><b>${r.correctIds.length}</b>${c.unique}</span></div>${r.wrongIds.length?`<h4>${c.review}</h4>${wordsMarkup(s,r.wrongIds)}`:`<p>${c.noWrong}</p>`}${growth(s)}${button("destination",c.changeRegion,"",true)}</section>`;
  }
