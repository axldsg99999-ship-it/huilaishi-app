(function () {
  ...
  function learningCollectionName(level) {
    const names = locale() === "zh"
      ? ["生存开口", "日常基础", "旅行社交", "工作生活", "观点表达", "文化进阶"]
      : ["เอาตัวรอด", "ชีวิตประจำวัน", "เที่ยวและเข้าสังคม", "งานและชีวิต", "แสดงความคิดเห็น", "ภาษาและวัฒนธรรม"];
    return names[Math.max(0, Math.min(5, (Number(level) || 1) - 1))];
  }
  ...
  const COPY = {
    zh: {
      ...
      learningBridgeTitle: level => `${learningCollectionName(level)} 学习进度`, learningSeen: "已练", learningDue: "待复习", learningWrong: "错词", learningMastered: "已掌握", learningReview: count => count ? `复习 ${count} 个错词` : "复习到期词", monsterCollection: (chapter, total) => `本章 ${chapter}/${MONSTERS_PER_CHAPTER} · 136 怪图鉴 ${total}/${ALL_MONSTER_CONFIGS.length}`, learningSaved: count => `本局 ${count} 次词汇判断已写回学习进度`,
      ...
      games: {
        ...
        monster: ["02 · 136 怪 · 双地图", "校园声斗赛", "36 关主线之外新增百怪巡游：10 区、100 只主动技能怪，关卡进度、词库与图鉴分别保存。", "打"],
      ...
      audioLoading: "正在查找本机学习声包…", audioUnavailable: level => `${learningCollectionName(level)} 学习声包尚未安装，无法保证清晰示范音。`, audioFailed: "音频加载失败，请检查声包后重试。",
      installPack: level => `安装 ${learningCollectionName(level)} 声包`, useText: "先用文字模式", textPrompt: "看词选出正确意思", textFallbackReady: "已切换为看词选义，本题仍可完成。",
      ...
      monsterVoice: "开口攻击", monsterVoiceHint: "转写匹配 + 速度 = 伤害", monsterListening: "正在听你说…", monsterJudging: value => value ? `设备转写：${value} · 正在核对` : "正在核对转写…", monsterVoicePass: score => `识别命中 · 匹配度 ${score}`, monsterVoiceFail: score => `匹配度 ${score} · 怪物反击`, monsterVoiceGradeLabel: "语音匹配", monsterVoiceGrades: { perfect: "高匹配", great: "匹配良好", pass: "识别命中" }, monsterVoiceGradeBonus: value => `语音匹配伤害 +${value}`, monsterNetwork: "允许本场联网识别", monsterFallback: "麦克风不可用时，可点下方意思继续战斗", monsterUnavailable: "这台设备暂时不能进行语音识别，请用下方文字招式继续。", monsterLocalMissing: "本机没有离线识别包；可允许系统语音服务在本场战斗中联网识别。", monsterFxOn: "战斗音效已开", monsterFxOff: "战斗音效已关",
      monsterReadyTitle: "选择路线，再进入多回合声斗", monsterReadyCopy: "主线保留 6 章 36 关；百怪巡游新增 10 区 100 只怪。每只怪保留独特技能；从简单词汇开始，关卡血量和时间压力逐步增加。角色不升级，答对记录写回词库。", monsterReadyRules: ["看技能预警选克制招", "每怪多次命中并叠连击", "通过关卡并记录练习"], monsterReadyDetails: "玩法与判定", monsterReadyOpponent: "本关对手", monsterStart: "进入当前关卡",
      monsterCampaignEyebrow: level => `校园声斗赛 · 第 ${level} 章`, monsterCampaignHook: "6 章 · 36 只怪 · 36 个独立关卡", monsterCampaignLoot: "从易到难 · 击败新怪，角色成长", monsterStartHint: "从本章尚未击败的关卡继续", monsterJudgeNote: "设备按最终转写判定命中，不等于母语教师发音评分。", monsterCombo: "连击", monsterMomentum: "声势", monsterMomentumTiers: { calm: "热身", heat: "连击升温", fever: "全场沸腾" }, monsterChallenge: "前三秒说准，触发暴击", monsterEntryBoss: "BOSS 登场", monsterEntryElite: "精英技能启动", monsterEntryRival: "新对手入场", monsterElite: "精英", monsterImpactLabels: { hit: "命中", critical: "精准暴击", burst: "纸片必杀", shield: "破盾", down: "击破", counter: "怪物反击" }, monsterYou: "你", monsterRoute: "本章六关路线", monsterCampaignMap: "36 关校园地图", monsterChapter: level => `第 ${level} 章`, monsterChapterProgress: value => `${value}/${MONSTERS_PER_CHAPTER}`, monsterGlobalStage: value => `第 ${value}/${MONSTER_CONFIGS.length} 关`, monsterCombatLevel: () => "", monsterEnemyLevel: value => `第 ${value} 关`, monsterLevelUp: () => "", monsterUpgradeRank: () => "", monsterComboDamage: value => `连击 ×${value.toFixed(2)}`, monsterDamageFormula: "速度与连击决定伤害，招式应对怪物技能", monsterFirstMission: "第一回合", monsterFirstSteps: "看预警 → 选招 → 听题 → 开口", monsterLearningGain: "词库进度 +1", monsterLearningReview: "已加入错词复习", monsterCutInKicker: "声势突破", monsterCutInTitle: "声浪突进",
      monsterModeTitle: "选择狩猎路线", monsterModeStory: "主线 36", monsterModeStoryHint: "六章学习地图", monsterModeRally: "百怪巡游 100", monsterModeRallyHint: "十区技能挑战", monsterRallyMap: "百怪巡游十区地图", monsterRallyZone: value => `巡游 ${value} 区`, monsterRallyZoneProgress: value => `${value}/${MONSTERS_PER_RALLY_ZONE}`, monsterRallyStage: value => `第 ${value}/${MONSTER_RALLY_CONFIGS.length} 只`, monsterRallyEyebrow: value => `百怪巡游 · 第 ${value} 区`, monsterRallyHook: "十区百怪 · 主动技能巡游", monsterRallyLoot: "按关卡渐进挑战，击败怪物点亮图鉴", monsterRallyStartHint: "从本区尚未击败的怪物继续", monsterRallyRoute: "本区十怪路线", monsterRallyRewardStreak: value => `本区已过 ${value}/${MONSTERS_PER_RALLY_ZONE}`, monsterRallyVictory: "本区十怪全破！", monsterRallyVictoryCopy: "本区 10 只怪已全部完成，关卡、图鉴和词库记录都已保存。", monsterTotalCollection: (story, rally) => `主线 ${story}/${MONSTER_CONFIGS.length} · 巡游 ${rally}/${MONSTER_RALLY_CONFIGS.length}`,
      ...
      monsterRewardTitle: "这一关，通过", monsterRewardCopy: "选一项强化，带着成长进入下一关。", monsterRewards: { power: ["力量贴纸", "本次远征伤害倍率 +10%"], guard: ["体能绷带", "生命上限 +12，并恢复 18"], tempo: ["时间书签", "之后每题多 0.5 秒"] }, monsterRewardPick: "关卡完成", monsterRewardHp: (value, max) => `远征体力 ${value}/${max}`, monsterRewardStreak: value => `本章进度 ${value}/${MONSTERS_PER_CHAPTER}`, monsterRewardNext: "下一关", monsterExitConfirm: "远征已保存到本机。退出后可继续，保留体力、怪物血量和连击；尚未完成的题会重新开始。现在退出吗？",
      monsterResumeTitle: "你的远征还在", monsterResume: "继续远征", monsterResumeStats: (hp, max) => `体力 ${hp}/${max}`, monsterResumeNote: "从最近结算的回合继续 · 仅保存在当前设备", monsterReplaceConfirm: "开启新挑战将替换未完成的回合；已解锁图鉴与词库进度保留。确定开启吗？", monsterStorageWarning: "当前无法持久保存。关闭页面后可能丢失本轮远征，请先检查浏览器存储空间。仍要退出吗？", monsterGrowthNote: "词汇由浅入深；击败新怪让角色成长，通关可选择强化", monsterSaveLocal: "回合已存本机", monsterSaveMemory: "仅暂存本页，关闭可能丢失",
    ...
    th: {
      ...
      learningBridgeTitle: level => `ความคืบหน้า ${learningCollectionName(level)}`, learningSeen: "ฝึกแล้ว", learningDue: "ถึงเวลาทบทวน", learningWrong: "คำที่พลาด", learningMastered: "จำได้แล้ว", learningReview: count => count ? `ทบทวนคำที่พลาด ${count} คำ` : "ทบทวนคำถึงกำหนด", monsterCollection: (chapter, total) => `บทนี้ ${chapter}/${MONSTERS_PER_CHAPTER} · สารานุกรม 136 ตัว ${total}/${ALL_MONSTER_CONFIGS.length}`, learningSaved: count => `บันทึกผลคำศัพท์ ${count} ครั้งกลับสู่ความคืบหน้าแล้ว`,
      ...
      audioLoading: "กำลังค้นหาชุดเสียงเพื่อเรียนในเครื่อง…", audioUnavailable: level => `ยังไม่ได้ติดตั้งชุดเสียงเพื่อเรียน ${learningCollectionName(level)} จึงเปิดเสียงตัวอย่างชัดเจนไม่ได้`, audioFailed: "โหลดเสียงไม่สำเร็จ โปรดตรวจชุดเสียงแล้วลองอีกครั้ง",
      installPack: level => `ติดตั้งชุดเสียง ${learningCollectionName(level)}`, useText: "ใช้โหมดข้อความก่อน", textPrompt: "ดูคำแล้วเลือกความหมายที่ถูก", textFallbackReady: "เปลี่ยนเป็นโหมดดูคำแล้ว ข้อนี้ยังเล่นต่อได้",
      ...
      voicePrompt: "ดูความหมาย แล้วพูดคำเป้าหมาย", voiceHint: "ฟังเสียงตัวอย่างก่อน แตะไมค์แล้วพูดให้ครบ ความตรงของคำถอดเสียงสุดท้ายต้องถึง 78 จึงพังประตู", voiceDemo: "ฟังเสียงตัวอย่างเพื่อเรียน", voiceStart: "เริ่มพูด", voiceListening: "กำลังฟัง…", voicePass: score => `ระบบรู้จำตรง · ${score} คะแนน`, voiceRetry: score => `ตรง ${score} คะแนน · ลองให้ชัดขึ้น`, voiceNetwork: "อนุญาตรู้จำออนไลน์ครั้งนี้", voiceLocalMissing: "เครื่องไม่มีชุดรู้จำออฟไลน์ อนุญาตบริการเสียงของระบบออนไลน์ในการต่อสู้นี้ได้", voiceUnavailable: "อุปกรณ์นี้สร้างคะแนนความตรงของคำถอดเสียงไม่ได้ โปรดใช้ Chrome/Safari ผ่าน HTTPS", voiceHeard: value => `คำถอดเสียง: ${value}`,
      ...
      monsterRewardTitle: "ผ่านด่านแล้ว", monsterRewardCopy: "เลือกพลังเสริมหนึ่งอย่าง แล้วก้าวสู่ด่านต่อไป", monsterRewards: { power: ["สติกเกอร์พลัง", "ตัวคูณดาเมจรอบนี้ +10%"], guard: ["ผ้าพันแผลพลัง", "HP สูงสุด +12 และฟื้น 18"], tempo: ["ที่คั่นเวลา", "ทุกข้อมีเวลาเพิ่ม 0.5 วิ"] }, monsterRewardPick: "จบด่าน", monsterRewardHp: (value, max) => `พลังเดินทาง ${value}/${max}`, monsterRewardStreak: value => `ความคืบหน้าบท ${value}/${MONSTERS_PER_CHAPTER}`, monsterRewardNext: "ด่านถัดไป", monsterExitConfirm: "บันทึกรอบแล้ว กลับมาเล่นต่อได้ ต้องการออกหรือไม่?",
      monsterResumeTitle: "การเดินทางยังรอคุณอยู่", monsterResume: "เล่นต่อ", monsterResumeStats: (hp, max) => `พลังชีวิต ${hp}/${max}`, monsterResumeNote: "ต่อจากรอบที่ตัดสินผลล่าสุด · เก็บเฉพาะเครื่องนี้", monsterReplaceConfirm: "เริ่มใหม่จะแทนที่รอบที่ยังไม่จบ แต่เก็บบันทึกคำศัพท์และด่านที่ผ่านไว้ เริ่มหรือไม่?", monsterStorageWarning: "ขณะนี้บันทึกถาวรไม่ได้ หากปิดหน้าอาจสูญเสียการเดินทางรอบนี้ โปรดตรวจสอบพื้นที่เก็บข้อมูล ยังต้องการออกหรือไม่?", monsterGrowthNote: "ผ่านด่านใหม่เพื่อพัฒนาตัวละคร และเลือกพลังเสริมระหว่างทาง", monsterSaveLocal: "บันทึกรอบนี้ในเครื่องแล้ว", monsterSaveMemory: "เก็บชั่วคราว ปิดหน้าอาจสูญหาย",
  ...
  function syncArcadeOrientationHint() {
    ...
    // Expedition supports both layouts: do not cover its answers with a rotate overlay.
    const flexibleLayout = sheet.dataset.arcadeGame === "rogue";
    hint.hidden = !document.body?.classList?.contains?.("arcade-game-active") || !portrait || !compact || orientationHintDismissed || monsterReady || flexibleLayout;
  ...
  // V128: retired levels and upgrade ranks remain accepted as legacy arguments only.
  ...
  function wordVoiceOptions(word, kind = "word") {
    ...
    return {
      ...
      hideTranscript: game?.type === "monster" && game.questionMode === "listen" && !game.answered,
      statusPresentation: game?.type === "monster" ? "scene" : "overlay",
      track: "standard"
    };
  }
  ...
  function recordLearningResult(word, correct, assisted = false) {
    if (!word?.id) return null;
    let snapshot = null;
    try {
      if (correct && assisted) snapshot = learningSnapshot(word.level);
      else if (game?.type === "monster" && window.VocabUI?.recordGamePractice) {
        game.learningRunId ||= globalThis.crypto?.randomUUID?.() || Date.now().toString(36) + "-" + Math.random().toString(36).slice(2);
        snapshot = window.VocabUI.recordGamePractice(word.id, Boolean(correct), `campus:${game.learningRunId}:${game.round}`);
      } else snapshot = window.VocabUI?.recordPracticeResult?.(word.id, Boolean(correct), `arcade:${game?.type || "unknown"}`) || null;
    } catch (_) {}
    if (!snapshot && !(correct && assisted)) {
      ...
      const promote = correct && (!current.lastGamePromotion || Date.now() - current.lastGamePromotion >= 86400000);
      const box = correct ? Math.min(5, Number(current.box || 0) + (promote && current.lastGameCorrect !== false ? 1 : 0)) || 1 : 1;
      const days = [0, 1, 3, 7, 14, 30];
      srs[word.id] = { box, seen: Number(current.seen || 0) + 1, due: correct ? Date.now() + days[box] * 86400000 : Date.now(), lastGamePromotion: promote || !correct ? Date.now() : Number(current.lastGamePromotion || 0), lastGameCorrect: correct };
      write("srs", srs);
      const wrong = new Set(read("wrong", []));
  ...
  function campusRouteMarkup() {
    ...
    }).join("")}</ol><p>${zh ? "从常用词逐步挑战更难的关卡；答题记录回到词库，错词继续复习。" : "เริ่มจากคำพื้นฐานไปสู่ด่านที่ยากขึ้น ผลคำตอบบันทึกในคลังคำศัพท์ และทบทวนคำที่ตอบผิด"}</p></section>`;
  }

  function nextMonsterChapter() {
    const saved = readMonsterExpedition();
    if (saved?.monsterMode === "story" && ["battle", "reward"].includes(saved.runPhase)) return saved.campaignLevel;
    for (let chapter = 1; chapter <= MONSTER_CHAPTER_COUNT; chapter += 1) {
      if (!monsterCollectionSnapshot(chapter).chapterCleared) return chapter;
    }
    return 1; // All cleared: replay without losing the collection.
  }
  ...
  // Difficulty belongs to the stage, never to how much XP the player has farmed.
  function monsterStageDifficulty(monster = currentMonster()) {
    return { stage: monsterGlobalStage(monster), hp: monsterScaledHp(monster),
      turnMs: monster?.turnMs || MONSTER_TURN_MS,
      missDamage: Number(monster?.counterDamage) || 12, timeoutDamage: Number(monster?.timeoutDamage) || 16 };
  }
  ...
  function normalizeMonsterExpedition(value, dir = direction()) {
    ...
    const playerHp = value.rules === "stages-v128" && Number(value.playerMaxHp) > 0
      ? Math.min(playerMaxHp, Math.ceil(int(value.playerHp) / Number(value.playerMaxHp) * playerMaxHp)) : int(value.playerHp, playerMaxHp);
    const monsterMaxHp = monsterScaledHp(monster);
    const monsterHp = value.rules === "stages-v128" && Number(value.monsterMaxHp) > 0
      ? Math.min(monsterMaxHp, Math.ceil(int(value.monsterHp) / Number(value.monsterMaxHp) * monsterMaxHp)) : int(value.monsterHp, monsterMaxHp);
    ...
    return {
      version: 1, rules: "growth-v129", direction: dir, updatedAt: int(value.updatedAt, Number.MAX_SAFE_INTEGER),
      ...
      learningRunId: /^[a-zA-Z0-9-]{1,80}$/.test(value.learningRunId || "") ? value.learningRunId : "legacy-" + int(value.updatedAt, Number.MAX_SAFE_INTEGER),
  ...
  function renderCampusPreparation() {
    ...
    q("#arcade-stage").innerHTML = `<section class="campus-prep" aria-labelledby="campus-prep-title">
      ...
      <div class="campus-prep-page"><div class="campus-prep-scroll" tabindex="0" role="region" aria-label="${zh ? '本关词汇，可上下滑动' : 'คำศัพท์ของด่าน เลื่อนขึ้นลงได้'}"><header><span class="campus-prep-stamp">${String(game.monsterIndex + 1).padStart(2, "0")} / 06</span><p>${zh ? "先认识，再出招" : "รู้จักคำก่อน แล้วค่อยออกท่า"}</p><h3 id="campus-prep-title">${esc(zh ? lesson.zh : lesson.th)}</h3><p>${esc(zh ? lesson.sceneZh : lesson.sceneTh)}</p></header>
      ...
      <small class="campus-prep-note">${zh ? "上滑查看全部词汇 · 小测不计时，不评发音。" : "เลื่อนดูคำศัพท์ทั้งหมด · ไม่จับเวลา ไม่ประเมินการออกเสียง"}</small></div><footer class="campus-prep-actions">
  ...
  function renderCampusCheck() {
    ...
    q(".campus-prep-scroll").scrollTop = 0;
    q(".campus-prep-page").dataset.checking = "true";
  ...
  function renderHall() {
    const c = copy();
    const rogueEntry = q("#rogue-entry");
    if (rogueEntry) {
      rogueEntry.disabled = false;
      q("#rogue-entry-title").textContent = locale() === "zh" ? "双城远征" : "ผจญภัยสองเมือง";
      q("#rogue-entry-kicker").textContent = locale() === "zh" ? "单人肉鸽 · 六档难度" : "โร้กไลต์คนเดียว · 6 ระดับ";
      q("#rogue-entry-note").textContent = locale() === "zh" ? "中泰主题关 · 随机强化 · 词汇与长句" : "ด่านธีมจีนและไทย · พลังเสริมสุ่ม · คำศัพท์และประโยคยาว";
    }
  ...
  async function openRogue(options = {}) {
    closeGame();
    const request = ++rogueRequest, dir = direction();
    const entry = q("#rogue-entry");
    if (entry) entry.disabled = true;
    try {
      // The journaled learning bridge must exist before any rewarded answers.
      if (typeof ensureFeatureBundle === "function") await ensureFeatureBundle("vocab");
      if (request !== rogueRequest || dir !== direction()) return;
      if (!window.VocabUI?.recordGamePractice || !window.HUILAISHI_ROGUE_UI) throw new Error("rogue-resources-unavailable");
      window.HUILAISHI_ROGUE_WORLD.configure(corpus(), ALL_MONSTER_CONFIGS, MONSTER_RALLY_ZONES);
      const hero = MONSTER_HERO_CONFIGS.find(h => h.id === (dir === "zh-th" ? "chinese" : "thai"));
      game = { type: "rogue", direction: dir, current: null };
      const sheet = q("#arcade-sheet"), stage = q("#arcade-stage");
      delete sheet.dataset.campusChapter;
      sheet.dataset.arcadeGame = "rogue";
      q("#arcade-sheet-kicker").textContent = dir === "zh-th" ? "折声界 · 双界恋歌" : "โลกเสียงพับ · บทเพลงรักสองโลก";
      q("#arcade-sheet-title").textContent = dir === "zh-th" ? "双城远征" : "ผจญภัยสองเมือง";
      hideFeedback();
      stage.setAttribute("aria-live", "off");
      openSheet("arcade-sheet");
      setGameOrientation(true);
      window.HUILAISHI_ROGUE_UI.open({
        guided: options.guided === true, rehearsal: options.rehearsal === true, direct: options.direct === true,
        onGuideDone: typeof options.onGuideDone === "function" ? options.onGuideDone : () => { window.closeSheets?.(); if (typeof returnToHub === "function") returnToHub(); },
        stage, sheet, dir, hero, monsters: ALL_MONSTER_CONFIGS, words: corpus(),
        impact: kind => playMonsterImpactSound(kind),
        journey: window.HUILAISHI_CAMPUS_ADVENTURE.journey(dir), storage: globalThis.HUILAISHI_STORAGE,
        reviewIds: learningSnapshot(1).dueIds || [],
        progress: level => learningSnapshot(level),
        canEnter: profile => profile.id === "primer" || (profile.level === 1 && !profile.id.startsWith("rally-")) || canUseFullMonsterRoute(),
        requestAccess: () => { window.closeSheets?.(); requestPremium("monster-full"); },
        learn: (id, correct, attempt) => window.VocabUI.recordGamePractice(id, correct, attempt),
        play: (word, playback) => { if (game?.type !== "rogue") return false; game.current = word; return playWordVoice(word, playback); },
        onExit: () => window.closeSheets?.()
      });
    } catch (_) {
      window.showToast?.(dir === "zh-th" ? "远征资源未准备好，请重试" : "ไฟล์ผจญภัยยังไม่พร้อม กรุณาลองใหม่");
    } finally { if (entry) entry.disabled = false; }
  }

  function openGame(type) {
    rogueRequest++;
    window.HUILAISHI_ROGUE_UI?.close?.();
    clearTimers();
  ...
  async function playWordVoice(word, playback = {}) {
    ...
    try {
      ...
      if (bundled) {
        ...
        setAudioStatus();
        engine.speak(view.target, { ...options, lang: view.voiceLang, rate: .78, onStart: playback.onStart, onEnd: playback.onEnd, onError: playback.onError });
        return true;
      }
  ...
  function renderMatchReady() {
    ...
    setProgress(0);
    q("#arcade-stage").innerHTML = `<div class="arcade-prompt match-ready" role="group" aria-labelledby="arcade-match-ready-title"><span class="game-chip">${learningCollectionName(activeLevel())} · MATCH</span><h3 id="arcade-match-ready-title">${esc(c.matchReadyTitle)}</h3><p>${esc(c.tapPair)}</p><span class="meaning-hint">${esc(c.matchReadyCopy)}</span><button type="button" id="arcade-match-start" data-match-start>${esc(c.matchStart)}</button></div>`;
  ...
  function makeCampusThoughtOptions(word) {
    const lesson=isCampusChapter()&&window.HUILAISHI_CAMPUS_CURRICULUM?.encounter(corpus(),game.monsterIndex);
    if(!lesson)return makeWordOptions(word);
    const correct=wordView(word),seen=new Set([correct.meaning]);
    const count=game.monsterIndex===0?2:game.monsterIndex===1?3:4;
    const candidates=[...lesson.words,...lesson.previous].filter(item=>{
      const meaning=wordView(item).meaning;
      if(seen.has(meaning))return false;seen.add(meaning);return true;
    });
    return shuffle([{word,view:correct,correct:true},...shuffle(candidates).slice(0,count-1).map(item=>({word:item,view:wordView(item),correct:false}))]);
  }
  ...
  function renderWordQuestion() {
    ...
    q("#arcade-stage").innerHTML = `<div class="arcade-prompt"><span class="game-chip">${learningCollectionName(activeLevel())} · ${fallback ? "TEXT" : "AUDIO"}</span>${prompt}<small id="arcade-audio-status" role="status" aria-live="polite"></small></div><div class="arcade-options">${game.options.map((option, index) => `<button class="arcade-option" data-answer="${index}"><span>${c.answerLetters[index]}</span><span class="arcade-option-copy"><b>${esc(option.view.meaning)}</b></span></button>`).join("")}</div>`;
  ...
  function renderVoiceGateQuestion() {
    ...
    q("#arcade-stage").innerHTML = `<div class="arcade-prompt arcade-voice-gate" data-voice-gate-state="ready">
      <span class="game-chip">${learningCollectionName(activeLevel())} · SPEAK TO UNLOCK</span>
  ...
  function renderSpeedQuestion() {
    ...
    q("#arcade-round").textContent = `${game.round + 1} · ${game.streak}× COMBO`;
    q("#arcade-timer").textContent = c.time(game.seconds);
    q("#arcade-stage").innerHTML = `<div class="arcade-prompt"><span class="game-chip">${learningCollectionName(activeLevel())} · SPEED</span><h3 lang="${view.lang}">${esc(view.target)}</h3><p>${esc(view.reading)}</p>${phoneticHintMarkup(view.phoneticHint)}<span class="meaning-hint">${esc(c.speedPrompt)}</span></div><div class="arcade-options">${game.options.map((option, index) => `<button class="arcade-option" data-answer="${index}"><span>${c.answerLetters[index]}</span><span class="arcade-option-copy"><b>${esc(option.view.meaning)}</b></span></button>`).join("")}</div>`;
  }

  function currentMonsterHero(heroId = game?.monsterHeroId || DEFAULT_MONSTER_HERO_ID) {
    const base = MONSTER_HERO_CONFIGS.find(hero => hero.id === heroId) || MONSTER_HERO_CONFIGS[0];
    return { ...base, ...globalThis.XULONG_CAMPUS_REWARDS?.appearance(base.id) };
  }
  ...
  function monsterHeroName(hero = currentMonsterHero()) {
    if (hero?.id === "chinese") return locale() === "zh" ? "小艾 · 中国搭档" : "小艾 · คู่หูชาวจีน";
    if (hero?.id === "thai") return locale() === "zh" ? "CHANINDA · 泰国搭档" : "CHANINDA · คู่หูชาวไทย";
  ...
  function monsterRoundResultModel(data = {}) {
    ...
    if (correct) {
      ...
      if (Number(data.powerMultiplier) > 1) chips.push(c.monsterRoundUpgrade(Number(data.powerMultiplier).toFixed(2)));
    ...
    q("#arcade-stage").innerHTML = `<section class="arcade-monster-ready arcade-monster-ready-v79${campus ? " campus-ready" : ""}" aria-labelledby="monster-ready-title">
      ...
      <div class="arcade-monster-ready-brief">
        ...
        <div class="arcade-monster-ready-headline">
          ...
          <p class="arcade-expedition-growth"><span>${esc(c.monsterGrowthNote)}</span></p>
          <p class="arcade-monster-ready-opponent"><span>${esc(c.monsterReadyOpponent)} · ${esc(monsterStageLabel(monster))}</span><b>${esc(monsterName(monster))} · ${monsterScaledHp(monster)} HP</b><em>${esc(monsterTraitText(monster))}</em></p>
        </div>
  ...
  function renderCampusLobby(focusSelector) {
    ...
    q("#arcade-stage").innerHTML = `<section class="campus-ready campus-lobby" data-restored="${progress.chapterCleared}" style="--journey-scene:url('${esc(journey.art)}')">
      <div class="campus-lobby-scene"><span class="campus-location">${esc(journey.location)}</span><div class="campus-lobby-title"><small>${esc(journey.subtitle)}</small><h3>${esc(journey.title)}</h3><p>${esc(progress.chapterCleared ? journey.restored : journey.intro)}</p></div><div class="campus-lobby-cast" aria-hidden="true"><img src="${esc(hero.art)}"/><img src="${esc(partner.art)}"/></div><span class="campus-scene-caption">${zh ? "身在两座城，回声仍能一起出招。" : "อยู่คนละเมือง แต่เสียงสะท้อนยังช่วยกันออกท่าได้"}</span>${progress.chapterCleared ? '<div class="campus-restored-flags" aria-hidden="true"><i></i><i></i><i></i><i></i><i></i><i></i></div>' : ''}</div>
      ...
      <p class="campus-duo-note">${zh ? "两位搭档基础能力相同；另一位负责合击支援。" : "คู่หูมีพลังพื้นฐานเท่ากัน อีกคนช่วยด้วยท่าคู่หู"}</p>
      ...
      <details class="campus-instructions"><summary>${zh ? "第一次冒险？查看玩法" : "ผจญภัยครั้งแรก? ดูวิธีเล่น"}</summary><p>${zh ? "先选招式，再听题。听完才计时；可以开口，也可以选答案。正确回答积攒合击，击败对手后关卡完成。第 3、5 站前会出现路线事件。" : "เลือกท่าแล้วฟังโจทย์ เริ่มจับเวลาหลังฟังจบ พูดหรือเลือกคำตอบก็ได้ ตอบถูกเพื่อชาร์จท่าคู่ ชนะแล้วไปด่านถัดไป มีเหตุการณ์ทางเลือกก่อนด่าน 3 และ 5"}</p><small>${esc(copy().monsterJudgeNote)}</small></details>
  ...
  function monsterStyleStatsMarkup(seconds, damage) {
    const zh = locale() === "zh";
    return `<span>${esc(seconds)} ${zh ? "秒" : "วิ"}</span><span>${zh ? "攻击" : "โจมตี"} ${esc(damage)}</span>`;
  }
  ...
  function monsterStyleButtonsMarkup(monster = currentMonster()) {
    ...
    return MONSTER_STYLE_ORDER.map(styleId => {
      ...
      return `<button type="button" class="${recommended ? "is-skill-counter" : ""}" data-monster-arm="${styleId}" data-monster-style="${styleId}" aria-label="${esc(aria)}" aria-pressed="false"><i aria-hidden="true">${esc(style.icon)}</i><span><b>${esc(labels[0])}</b><small data-monster-style-stats>${monsterStyleStatsMarkup(seconds, damage)}</small><em data-monster-style-tag>${esc(tag)}</em></span></button>`;
    }).join("");
  }
  ...
  function updateMonsterPlanningUi() {
    ...
    [...document.querySelectorAll("[data-monster-style]")].forEach(button => {
      ...
      if (value) value.innerHTML = monsterStyleStatsMarkup(seconds, damage);
    ...
    if (voiceLabel) voiceLabel.textContent = game.timerActive ? c.monsterVoice : cueing ? c.monsterCueShort : game.cueFailed ? c.monsterTextStart : game.questionMode === "read" ? (locale() === "zh" ? `看字 · 开始 ${seconds} 秒` : `อ่านคำ · เริ่ม ${seconds} วิ`) : c.monsterArm(seconds);
    ...
    window.XULONG_THOUGHT_BATTLE?.sync(q("#arcade-stage"), game, locale());
  }
  ...
  function updateMonsterHud() {
    ...
    if (damageFormulaValue) damageFormulaValue.textContent = `${copy().monsterComboDamage(monsterComboMultiplier(streak))}`;
    if (playerHealth) playerHealth.setAttribute("aria-label", `${copy().monsterPlayer} ${playerHp}/${playerMaxHp}`);
  ...
  function revealCampusTarget(includeReading = true) {
    const node = q("[data-campus-target]");
    if (node && game?.current) {
      const view = wordView(game.current);
      node.innerHTML = `<h3 lang="${view.lang}">${esc(view.target)}</h3>${includeReading ? `<p>${esc(view.reading)}</p>` : ""}`;
    }
  }

  function reconcileCampusRewards() {
    const model = globalThis.XULONG_CAMPUS_REWARDS;
    if (!model) return null;
    const story = readMonsterCollection(), rally = readMonsterRallyCollection();
    const ids = ALL_MONSTER_CONFIGS.filter(m => m.rally
      ? rally.zones?.[String(m.rallyZone)]?.defeated?.includes(m.id)
      : story.chapters?.[String(m.chapter)]?.defeated?.includes(m.id)).map(m => m.id);
    return model.reconcile(direction(), "clear", ids);
  }

  function campusRewardReceiptMarkup() {
    const result = reconcileCampusRewards();
    if (!result?.ok) return "";
    const state = result.state, zh = locale() === "zh";
    const entry = direction()+":clear:"+currentMonster().id;
    if (!state.receipts.includes(entry)) return "";
    return `<div class="campus-earned"><b>${zh ? "本关首通奖励 · 30 点" : "รางวัลผ่านครั้งแรก · 30 แต้ม"}</b><span>${zh ? "通关 20 + 角色成长 10 · 已计入衣橱" : "ผ่านด่าน 20 + เติบโต 10 · บันทึกในตู้เสื้อผ้า"}</span><small>${zh ? "余额 "+state.balance+" · 每关每个语言方向仅发一次" : "คงเหลือ "+state.balance+" · แต่ละด่านและเส้นทางรับได้ครั้งเดียว"}${result.durable ? "" : (zh ? " · 本次会话有效" : " · เฉพาะช่วงนี้")}</small></div>`;
  }

  function renderMonsterQuestion() {
    if (!game || game.type !== "monster") return;
    if (renderCampusFork()) return;
    if (isCampusChapter() && game.stageRound === 0 && game.campusPreparedMonsterId !== currentMonster().id && renderCampusPreparation()) return;
    q("#arcade-sheet")?.style?.setProperty?.("--journey-scene", `url("${campusSceneArt()}")`);
    game.runPhase = "battle";
    saveMonsterExpedition();
    game.cueFailed = false;
    game.thoughtsReady = false;
    game.cueStarted = false;
    game.inputNotice = false;
    ...
    const view = wordView(word);
    game.current = word;
    game.answerAssisted = false;
    game.questionMode = isCampusChapter() && (game.monsterIndex < 2 || game.stageRound % 2 === 0) ? "listen" : "read";
    game.options = makeCampusThoughtOptions(word);
    ...
    setProgress(monsterProgress());
    q("#arcade-stage").innerHTML = `<div class="arcade-monster-world is-clear-stage ${monster.rally ? "is-rally" : ""} ${monster.elite ? "is-elite" : ""} ${skillState?.active ? "is-skill-active" : ""} ${monster.boss ? "is-boss" : ""} ${monsterIsEnraged(monster) ? "is-enraged" : ""} ${game.rhythmWindow ? "is-rhythm-turn" : ""} ${game.resonanceWindow ? "is-resonance-turn" : ""} ${game.burstArmed ? "is-burst-armed" : ""}" data-monster-id="${monster.id}" data-monster-mode="${monster.rally ? "rally" : "story"}" data-monster-chapter="${monster.chapter}" data-monster-zone="${monster.rallyZone || 0}" data-monster-motion="${monster.motion || "spring"}" data-monster-skill="${monster.skill?.id || "none"}" data-monster-relic="${monster.relic?.kind || "none"}" data-monster-variant="${Number(monster.artVariant) || 0}" data-skill-state="${skillState?.active ? "active" : "cooldown"}" data-monster-hero="${hero.id}" data-campus-outfit="${hero.outfitId || ""}" data-monster-mechanic="${monster.trait || "none"}" data-player-style="${game.monsterStyle}" data-monster-state="${monsterEntering ? "enter" : "ready"}" data-action-frame="idle" data-combo-tier="${monsterMomentumTier()}" data-first-turn="${String(firstTurn)}" style="--monster:${monster.color};--monster-accent:${monster.accent};--monster-scene:${monster.scene};--monster-ground:${monster.ground};--hero-accent:${hero.accent}">
      <div class="arcade-monster-hud">
        <div class="arcade-monster-health is-player" aria-label="${esc(c.monsterPlayer)} ${game.playerHp}/${game.playerMaxHp}"><div class="arcade-player-tag"><b>${esc(monsterHeroName(hero).split(" · ")[0])}</b><small>${esc(c.monsterCombo)} <em data-monster-combo>${Math.max(0, Number(game.streak) || 0)}</em></small></div><span><strong data-monster-player-hp>${game.playerHp}/${game.playerMaxHp}</strong></span><i><em data-monster-player-bar style="width:${game.playerHp / game.playerMaxHp * 100}%"></em></i></div>
        <div class="arcade-monster-health is-enemy" aria-label="${esc(c.monsterEnemy)} ${game.monsterHp}/${game.monsterMaxHp}"><div class="arcade-monster-name"><small>${esc(monster.boss ? `${c.monsterBoss} · ${monsterStageLabel(monster)}` : monster.elite ? `${c.monsterElite} · ${monsterStageLabel(monster)}` : monsterStageLabel(monster))}</small><b>${esc(monsterName(monster))}</b><em data-monster-trait>${esc(monsterTraitText(monster))}</em></div><span><strong data-monster-enemy-hp>${game.monsterHp}/${game.monsterMaxHp}</strong></span><i><em data-monster-enemy-bar style="width:${game.monsterMaxHp > 0 ? game.monsterHp / game.monsterMaxHp * 100 : 0}%"></em></i></div>
      ...
      <div class="arcade-monster-battlefield">
        ...
        ${hero.id === "thai" ? `<div class="arcade-monster-cut-in" aria-hidden="true"><img src="${esc(hero.outfitId === "thai-varsity" ? hero.frames.strike : MONSTER_THAI_STRIKE_CUTIN)}" alt="" draggable="false" decoding="async" /><span><small>${esc(c.monsterCutInKicker)}</small><b>${esc(c.monsterCutInTitle)}</b></span></div>` : ""}
      ...
      <div class="arcade-monster-time"><i><em data-monster-time-bar style="width:100%"></em></i><b><span>${esc(c.monsterRule)}<small data-monster-damage-formula>${esc(c.monsterComboDamage(monsterComboMultiplier(game.streak)))}</small></span><strong data-monster-power>${esc(c.monsterChoosePower)}</strong></b></div>
      <p class="arcade-monster-status" data-monster-status role="status" aria-live="assertive">${esc(c.monsterReady)}</p>
    </div>
    <div class="arcade-monster-command${firstTurn ? " is-first-turn" : ""}" data-command-phase="plan" data-question-mode="${game.questionMode}">
      ...
      <div class="arcade-monster-question"><span class="game-chip">${esc(monsterStageLabel(monster))} · ${esc(game.questionMode === "listen" ? (locale() === "zh" ? "听音破招" : "ฟังเสียงเพื่อออกท่า") : (locale() === "zh" ? "认字施法" : "อ่านคำเพื่อร่ายเวท"))}</span><div class="arcade-monster-word"><button type="button" data-monster-audio disabled aria-label="${esc(c.monsterHear)}"><svg><use href="#i-volume"></use></svg></button><div data-campus-target>${game.questionMode === "listen" ? '<h3 class="campus-listen-mark" aria-label="'+esc(locale() === "zh" ? "听完声音，找出意思" : "ฟังเสียงแล้วเลือกความหมาย")+'">♪</h3><p>'+esc(locale() === "zh" ? "听声音，找意思" : "ฟังแล้วเลือกความหมาย")+'</p>' : '<h3 class="campus-listen-mark" aria-hidden="true">字</h3><p>'+esc(locale() === "zh" ? "准备好后，看字找意思" : "พร้อมแล้ว อ่านคำและเลือกความหมาย")+'</p>'}</div></div><span class="meaning-hint">${esc(c.monsterPrompt)}</span></div>
    ...
    window.XULONG_THOUGHT_BATTLE?.mount(q("#arcade-stage"), locale());
    updateMonsterPlanningUi();
    ...
    globalThis.requestAnimationFrame?.(() => q("#arcade-stage [data-thought-cue], #arcade-stage [data-monster-voice]")?.focus?.({ preventScroll: true }));
  }

  function startMonsterTimer(audioFailed = false) {
    if (!game || game.type !== "monster" || game.answered || game.timerActive) return;
    game.busy = false;
    game.thoughtsReady = true;
    game.timerActive = true;
    if (audioFailed) { game.questionMode = "read"; game.answerAssisted = true; }
    if (game.questionMode === "read") revealCampusTarget(false);
    game.remainingMs = game.turnMs;
    ...
    if (audioFailed) {
      ...
      if (tag) tag.textContent = `${monsterStageLabel()} · ${locale() === "zh" ? "文字题" : "โจทย์ข้อความ"}`;
    }
    clearInterval(timerId);
    timerId = setInterval(updateMonsterTimer, 100);
    updateMonsterTimer();
    updateMonsterPlanningUi();
    q("[data-monster-answer]")?.focus?.({ preventScroll: true });
  }

  function beginMonsterTurn(styleId = "steady") {
    if (!game || game.type !== "monster" || game.answered || game.timerActive || game.busy || game.thoughtsReady && !game.cueFailed) return;
    if (game.cueFailed) {
      game.cueFailed = false;
    ...
    if (game.questionMode === "read") {
      const active=game,round=game.round;
      game.busy=true;game.thoughtsReady=true;
      revealCampusTarget(false);updateMonsterPlanningUi();
      schedule(()=>{if(game===active&&game.round===round&&!game.answered)startMonsterTimer(false);},MONSTER_INPUT_GRACE_MS);
      return;
    }
    const active = game;
    const round = game.round;
    ...
    // Cold audio preparation and actual playback have separate deadlines.
    // A long sentence that has started playing must not be cut off at 7 s.
    let deadline=Date.now()+12000;
    const watchdog=()=>{
      if(completed||game!==active||game.round!==round||game.answered)return;
      const remaining=deadline-Date.now();
      if(remaining<=0)finish(true);else schedule(watchdog,Math.min(remaining,1000));
    };
    schedule(watchdog,1000);
    void playWordVoice(game.current, {
      onStart: event=>{
        if(completed||game!==active||game.round!==round)return;
        game.cueStarted=true;
        deadline=Date.now()+Math.min(90000,Math.max(15000,(Number(event?.duration)||0)*1500+5000));
        updateMonsterPlanningUi();
      },
      onEnd: () => finish(false), onError: () => finish(true)
    });
  }

  function replayCampusCue() {
    if (!game || game.type !== "monster" || !game.timerActive || game.answered || game.busy) return;
    const active=game,round=game.round;
    clearInterval(timerId);timerId=0;game.timerActive=false;game.busy=true;game.answerAssisted=true;game.thoughtsReady=false;game.cueStarted=false;
    setMonsterControlsDisabled(true);updateMonsterPlanningUi();
    const status=q("[data-monster-status]");
    if(status)status.textContent=locale()==="zh"?"辅助重听 · 不扣时间，不计速度加成与独立回忆":"ฟังซ้ำแบบช่วยเหลือ · ไม่เสียเวลา ไม่ได้โบนัสความเร็วหรือบันทึกว่าจำเองได้";
    let done=false;
    const finish=(failed=false)=>{
      if(done||game!==active||game.round!==round||game.answered)return;done=true;
      if(failed){
        wordAudioRequest++;try{window.HUILAISHI_SPEECH?.stop?.();}catch(_){}
        game.busy=false;game.cueFailed=true;game.thoughtsReady=false;
        if(status)status.textContent=copy().monsterCueFailed;
        const recovery=q('[data-monster-audio-recovery]');if(recovery)recovery.hidden=false;
        updateMonsterPlanningUi();return;
      }
      game.busy=false;game.thoughtsReady=true;updateMonsterPlanningUi();
      schedule(()=>{if(game===active&&game.round===round&&!game.answered)startMonsterTimer(false);},MONSTER_INPUT_GRACE_MS);
    };
    let deadline=Date.now()+12000;
    const watchdog=()=>{if(done||game!==active||game.round!==round||game.answered)return;
      const remaining=deadline-Date.now();if(remaining<=0)finish(true);else schedule(watchdog,Math.min(1000,remaining));};
    schedule(watchdog,1000);
    void playWordVoice(game.current,{onStart:event=>{
      if(done||game!==active||game.round!==round)return;game.cueStarted=true;
      deadline=Date.now()+Math.min(90000,Math.max(15000,(Number(event?.duration)||0)*1500+5000));
      updateMonsterPlanningUi();
    },onEnd:()=>finish(false),onError:()=>finish(true)});
  }
  ...
  async function attemptMonsterVoice(allowNetwork = false) {
    ...
    if (!scorer?.recognizeTarget) {
      ...
      game.inputNotice=true;updateMonsterPlanningUi();
      return;
    }
    game.inputNotice=false;game.voiceNotice="";game.voiceTranscript="";game.voicePhase="preparing";
    game.busy = true;
    ...
    try {
      ...
      maxMs: 7000,
      allowNetwork: allowNetwork || active.networkPermit,
      onStatus: value => { if(game!==active||game.round!==round)return;game.voicePhase=value;if (value === "listening" && !speechStartedAt) speechStartedAt = Date.now();window.XULONG_THOUGHT_BATTLE?.sync(q("#arcade-stage"),game,locale()); },
      onInterim: interim => {
        if (game !== active || game.round !== round || game.answered || !interim) return;
        game.voiceTranscript=interim.transcript||"";
    ...
    if (["local-missing", "network-consent"].includes(result.status)) {
      if (world) world.dataset.monsterState = "ready";
      game.voiceNotice = locale()==="zh" ? "本机识别不可用。授权本场联网识别后，语音可能交由系统服务商处理；也可点选作答。" : "รู้จำบนเครื่องไม่ได้ อนุญาตเครือข่ายได้ ผู้ให้บริการระบบอาจประมวลผลเสียง หรือเลือกคำตอบแทน";
      if (status) status.textContent = c.monsterLocalMissing;
      if (networkButton) networkButton.hidden = false;
      setMonsterControlsDisabled(false);
      game.inputNotice=true;
      resumeMonsterTimer(remainingAtSpeechStart);
      updateMonsterPlanningUi();
      return;
    }
    if (result.status !== "result" && result.status !== "passed") {
      game.voiceNotice = result.status==="not-allowed" ? (locale()==="zh"?"麦克风权限未开启，请在设置中允许后重试；也可点选作答。":"ยังไม่อนุญาตไมค์ เปิดสิทธิ์ในตั้งค่าหรือเลือกคำตอบแทน") : result.status==="cancelled" ? (locale()==="zh"?"已停止收音，可以点选或再试。":"หยุดรับเสียงแล้ว เลือกคำตอบหรือลองใหม่ได้") : c.monsterUnavailable;
      if (world) world.dataset.monsterState = "ready";
      if (status) status.textContent = c.monsterUnavailable;
      setMonsterControlsDisabled(false);
      game.inputNotice=true;
  ...
  function settleMonsterAnswer(index, timedOut = false, answerMeta = {}) {
    ...
    game.stageRound += 1;
    revealCampusTarget();
    recordLearningResult(game.current, correct, Boolean(game.answerAssisted || answerMeta.voice));
    ...
    if (learningPulse) {
      ...
      learningPulse.textContent = correct && (game.answerAssisted || answerMeta.voice) ? (locale() === "zh" ? "辅助练习 · 不计独立回忆" : "ฝึกแบบช่วยเหลือ · ไม่นับเป็นการจำเอง") : correct ? copy().monsterLearningGain : copy().monsterLearningReview;
    ...
    if (correct) {
      ...
      game.burstCharge = burstBonus ? 0 : Math.min(MONSTER_BURST_EVERY, (Number(game.burstCharge) || 0) + (game.answerAssisted ? 0 : voiceChargeGain));
      game.burstArmed = false;
      const damageParts = monsterDamageParts(game.answerAssisted ? 0 : game.remainingMs, game.streak, game.turnMs, game.combatLevel, game.rewardRanks?.power);
      ...
      game.correct += 1;
      if (!game.answerAssisted) game.streak += 1;
      game.bestStreak = Math.max(game.bestStreak, game.streak);
  ...
  function renderMonsterReward() {
    ...
    const aftermath = campusAftermathMarkup();
    q("#arcade-stage").innerHTML = `<section class="arcade-monster-reward stage-clear-page" aria-labelledby="stage-clear-title">
      <div class="stage-clear-result"><span class="stage-clear-stamp" aria-hidden="true">✓</span>
        <p class="stage-clear-eyebrow">${esc(monsterStageLabel(defeated))} · ${esc(monsterName(defeated))}</p>
        <h3 id="stage-clear-title">${esc(c.monsterRewardTitle)}</h3><p>${esc(c.monsterRewardCopy)}</p>
        <div class="stage-clear-record">${esc(locale() === "zh" ? `角色 ${game.combatLevel} 级` : `ระดับตัวละคร ${game.combatLevel}`)} · ${esc(rewardProgress)} · ${esc(game.saveDurable ? c.monsterSaveLocal : c.monsterSaveMemory)}</div>
        ${campusRewardReceiptMarkup()}
        ${aftermath ? `<details class="stage-clear-story"><summary>${locale() === "zh" ? "翻阅本关故事" : "อ่านเรื่องราวของด่านนี้"}</summary>${aftermath}</details>` : ""}
      </div>
      <div class="stage-clear-forward">
        ${next ? `<div class="stage-clear-opponent"><img src="${esc(next.art)}" alt="" draggable="false" decoding="async"><span><small>${esc(monsterStageLabel(next))}</small><b>${esc(monsterName(next))}</b><em>${esc(locale() === "zh" ? next.traitZh : next.traitTh)}</em></span></div>` : ""}
        <div class="stage-growth-choices" role="group" aria-label="${esc(locale() === "zh" ? "选择一项强化后进入下一关" : "เลือกพลังเสริมแล้วไปด่านถัดไป")}">${["power", "guard", "tempo"].map(id => {
          const labels = locale() === "zh" ? { power: ["攻击强化", "攻击倍率 +10%"], guard: ["体魄强化", "生命上限 +12 · 额外恢复"], tempo: ["从容施法", "本轮每题多 0.5 秒"] } : { power: ["เสริมการโจมตี", "ตัวคูณโจมตี +10%"], guard: ["เสริมพลังชีวิต", "พลังสูงสุด +12 · ฟื้นพลังเพิ่ม"], tempo: ["เพิ่มเวลา", "เพิ่มเวลาโจทย์ละ 0.5 วินาที"] };
          return `<button type="button" data-monster-reward="${id}"><span><b>${esc(labels[id][0])}</b><small>${esc(labels[id][1])}</small></span><i aria-hidden="true">→</i></button>`;
        }).join("")}</div>
      </div>
    </section>`;
  ...
  function finishGame() {
    ...
    const monsterPath = monsterResult
      ...
      : "";
    const learningPath = (finished.monsterVictory ? campusRewardReceiptMarkup() : "") + (finished.monsterVictory ? campusAftermathMarkup(true) : "") + campusLearningRecapMarkup() + (Number(finished.learningAttempts || 0) > 0
  ...
  function closeGame() {
    rogueRequest++;
    window.HUILAISHI_ROGUE_UI?.close?.();
    q("#arcade-stage")?.setAttribute?.("aria-live", "polite");
    clearTimers();
    document.body?.classList?.remove?.("arcade-monster-active");
  ...
  function bindEvents() {
    q("#rogue-entry")?.addEventListener("click", openRogue);
    ...
    q("#arcade-stage").addEventListener("click", event => {
      ...
      const thoughtCue = event.target.closest("[data-thought-cue]");
      if(thoughtCue && game?.type === "monster") {
        if(game.busy||game.answered||thoughtCue.disabled||game.thoughtsReady&&!game.timerActive&&!game.cueFailed)return;
        if(game.timerActive)return replayCampusCue();
        if(game.cueFailed){game.cueFailed=false;const recovery=q('[data-monster-audio-recovery]');if(recovery)recovery.hidden=true;}
        return beginMonsterTurn(game.monsterStyle||"steady");
      }
      const monsterAudio = event.target.closest("[data-monster-audio]"); if (monsterAudio && game?.type === "monster" && game.current) { replayCampusCue(); return; }
      ...
      const voiceCancel = event.target.closest("[data-monster-voice-cancel]");if(voiceCancel){window.PronunciationScorer?.cancelChallenge?.();return;}
  ...
  window.ArcadeUI = {
    ...
    requestClose,
    openRogue,
    openStoryChapter(level = nextMonsterChapter()) {
      openGame("monster");
