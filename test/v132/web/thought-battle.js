...
(function (root) {
  ...
  function mount(stage, language='zh') {
    ...
    const micIcon=world.querySelector('.arcade-monster-wave');
    if(micIcon)micIcon.innerHTML='<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="9" y="2" width="6" height="12" rx="3" fill="none"/><path d="M5 10v2a7 7 0 0 0 14 0v-2M12 19v3M8 22h8" fill="none" stroke="currentColor" stroke-width="1.8"/></svg>';
    const voiceStatus=make('p','thought-voice-status');voiceStatus.dataset.thoughtVoiceStatus='';voiceStatus.setAttribute('role','status');voiceStatus.hidden=true;world.append(voiceStatus);
    const cancel=make('button','thought-voice-stop');cancel.type='button';cancel.dataset.monsterVoiceCancel='';cancel.textContent=language==='th'?'หยุด':'停止';cancel.hidden=true;world.querySelector('.thought-microphone')?.append(cancel);
  ...
  function sync(stage, game, language='zh') {
    ...
    const voiceStatus=world.querySelector('[data-thought-voice-status]');
    if(voiceStatus){voiceStatus.hidden=!(p==='microphone'||game.inputNotice);voiceStatus.textContent=p==='microphone'?(game.voiceTranscript||(language==='th'?(game.voicePhase==='listening'?'กำลังฟังคุณพูด…':'กำลังเชื่อมต่อไมค์…'):(game.voicePhase==='listening'?'正在听你说…':'正在连接麦克风…'))):game.voiceNotice||c.micUnavailable;}
    const cancel=world.querySelector('[data-monster-voice-cancel]');if(cancel)cancel.hidden=p!=='microphone';
    if(mic){mic.disabled=game.busy||game.answered||(!game.timerActive&&p!=='failed');
      set('[data-monster-voice-label]',p==='failed'?c.text:p==='microphone'?(language==='th'?'กำลังรับเสียง…':'正在收音…'):p==='plan'?(language==='th'?'ฟังโจทย์ก่อนพูด':'先听题，再开口'):game.inputNotice?c.micRetry:(language==='th'?'แตะเพื่อพูด':'点击说话'));}
