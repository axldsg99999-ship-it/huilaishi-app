...
(function (root) {
  ...
  function start(state, seed, words, reviewIds = [], options = {}) {
    ...
    state.run = { id: `${seed >>> 0}-${state.meta.runs}`, seed: seed >>> 0, rng: seed >>> 0, phase: "map", floor: 0,
      ...
      rules: "growth-v129", profile,kind,difficulty:p?.level||1 };
    return true;
  }
  ...
  function restore(value, direction, validWordIds) {
    ...
    try {
      ...
      clean.run = clone(r);
      if (r.rules === "stages-v128") {
        const maxHp = 100 + (level(clean.meta) - 1) * 2 + (r.relics.includes("school-pin") ? 12 : 0);
        clean.run.hp = Math.min(maxHp, Math.ceil(r.hp / r.maxHp * maxHp));
        clean.run.maxHp = maxHp;
        clean.run.rules = "growth-v129";
      }
