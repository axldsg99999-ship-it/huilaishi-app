(() => {
  ...
  function wording() {
    ...
    if (isZh()) return {
      ...
      home: [
        ["故事从这里继续", "听声音、找线索、修复留言。练习就在剧情里，不必先打怪。"],
        ["旅行册随时翻", "只想练听力、认字或临摹，就从旅行册进入，不需要重看剧情。"],
        ["看看走过的地方", "沿河的足迹记录三处场景；中国与泰国路线分别保存。"],
        ["世界、战斗、成长", "战斗单独进行，词汇练习汇入共同的复习记录。成长页区分剧情和学习证据。"]
      ],
    ...
    return {
      ...
      progress: "{{current}} / {{total}}",
      home: [
        ["เดินทางต่อจากตรงนี้", "ฟังเสียง หาเบาะแส และเติมข้อความ การฝึกอยู่ในเรื่องราวโดยไม่ต้องต่อสู้ก่อน"],
        ["เปิดสมุดเดินทางได้เสมอ", "เลือกฝึกฟัง อ่าน หรือเขียนตามแบบ โดยไม่ต้องดูเนื้อเรื่องซ้ำ"],
        ["ดูรอยเท้าของคุณ", "สำรวจสามสถานที่ บันทึกเส้นทางจีนและไทยแยกกัน"],
        ["โลก ต่อสู้ และเติบโต", "การต่อสู้แยกจากเรื่องราว แต่ใช้บันทึกทบทวนคำศัพท์ร่วมกัน"]
      ],
      lesson: recognitionOnly ? [
  ...
  function launch(kind, { automatic = false } = {}) {
    ...
    const selectors = lesson
      ...
      : document.documentElement.classList.contains("wj-ready")
        ? [".wj-next", ".wj-world-tools [data-wj='notebook']", ".wj-world-tools [data-wj='map']", ".bottom-nav"]
  ...
  function init() {
    ...
    document.querySelector("#open-guide")?.addEventListener("click", () => {
      ...
      if (!home?.classList.contains("active") || !home.querySelector(".wj-next")) document.querySelector(".logo-button")?.click?.();
