(() => {
  ...
  const copy = {
    "zh-th": {
      title: "跟读反馈（测试版）",
      kicker: "跟读练习",
      note: "先听清晰示范，再完整说一遍。文字匹配只看设备最终候选，并给完整度、流利度、语速和相对音高走势；本功能是练习反馈，不是母语发音认证。",
      start: "开始跟读",
      stop: "说完了",
      recording: "正在听 · 说完整句",
      checking: "正在检查本机离线识别…",
      ...
      recordOnly: "只录音回放",
      networkReady: "声音可能发送给浏览器或系统语音服务商处理；不会发送到XULONG pasa服务器，只在本机临时内存保留供本次回放，关闭或刷新后清除。",
      ...
      pace: "语速",
      best: "本课最好",
      disclaimer: "文字分只比较设备最终候选与目标句；音高镜只比较有声音段的相对升降，不能精确判断词汇声调、长短元音、送气或口音。审核边界见「我的 > 内容与声音说明」。",
      pitchTitle: "声调 / 音高镜",
      pitchKicker: "本机音高对照",
      pitchScore: "走势接近",
    ...
    "th-zh": {
      ...
      networkReady: "เสียงอาจถูกส่งไปยังผู้ให้บริการรู้จำเสียงของเบราว์เซอร์หรือระบบ แต่จะไม่ถูกส่งไปยังเซิร์ฟเวอร์XULONG pasa และจะเก็บชั่วคราวในหน่วยความจำของเครื่องเพื่อฟังซ้ำครั้งนี้เท่านั้น เมื่อปิดหรือรีเฟรชหน้าจะถูกล้าง",
      ...
      pitchKicker: "เทียบระดับเสียงในเครื่อง",
  ...
  const ui = () => globalThis.HUILAISHI_NATIVE_ANDROID
    ? { ...copy[direction()], noRecognition: direction() === "zh-th"
      ? "本页暂只支持录音回听。新版安卓的文字识别请从主页“开口练”或战斗麦克风进入；识别匹配不等于发音评分。"
      : "หน้านี้รองรับการอัดเสียงฟังเอง หากต้องการรู้จำข้อความใน Android รุ่นใหม่ ให้เปิดฝึกพูดจากหน้าหลักหรือไมค์ในเกม การจับคู่ข้อความไม่ใช่คะแนนออกเสียง" }
    : copy[direction()];
  ...
  let nativeVoiceSerial = 0;
  function nativeVoiceBridge() {
    const bridge = globalThis.XulongNativeVoice;
    return bridge && typeof bridge.postMessage === "function" ? bridge : null;
  }

  function recognizeNative(options, target, lang, threshold, maxMs) {
    const bridge = nativeVoiceBridge();
    if (!bridge) return Promise.resolve({ passed:false, status:"none", score:0, transcript:"" });
    cancelChallenge();
    return new Promise(resolve => {
      const id = "voice-" + Date.now() + "-" + (++nativeVoiceSerial);
      let settled = false, timer = 0;
      const mode = options.allowNetwork ? "native-network" : "native-device";
      const previous = bridge.onmessage;
      const finish = result => {
        if (settled) return; settled = true; clearTimeout(timer);
        if (bridge.onmessage === receive) bridge.onmessage = previous;
        if (activeChallenge?.nativeId === id) activeChallenge = null;
        const value = { passed:false, score:0, transcript:"", confidence:0, target, threshold, mode, ...result };
        try { options.onStatus?.(value.status, value); } catch (_) {}
        resolve(value);
      };
      const cancel = () => {
        if (settled) return;
      ...
      const receive = event => {
        ...
        if (result.status === "preparing") { try { options.onStatus?.("preparing", {mode}); } catch (_) {} return; }
        if (result.status === "listening") { armDeadline(maxMs + 4000); try { options.onStatus?.("listening", {mode}); } catch (_) {} return; }
        if (result.status === "processing") { try { options.onStatus?.("processing", {mode}); } catch (_) {} return; }
        const transcript = typeof result.transcript === "string" ? result.transcript.slice(0,1000).trim() : "";
        if (result.status === "interim") { try { options.onInterim?.({transcript,final:false}); } catch (_) {} return; }
        if (result.status !== "result" || !transcript) { finish({status:result.status === "result" ? "no-speech" : result.status}); return; }
        const text = scoreText(target, transcript, lang);
        const score = Math.round(text.accuracy * .65 + text.completeness * .35);
        const passed = score >= threshold && !text.criticalMissing.length;
        finish({passed, status:passed?"passed":"result", transcript, score,
          accuracy:text.accuracy, completeness:text.completeness, criticalMissing:[...text.criticalMissing],
          confidence:Math.round(Math.max(0,Math.min(1,Number(result.confidence)||0))*100),
          metric:"primary-final-transcript-match"});
      };
      bridge.onmessage = receive;
      activeChallenge = {nativeId:id,cancel};
      armDeadline(30000);
      try { bridge.postMessage(JSON.stringify({action:"start",id,lang,allowNetwork:Boolean(options.allowNetwork),maxMs})); }
      catch (_) { finish({status:"start-failed"}); }
    });
  }

  async function recognitionMode(lang, allowNetwork = false) {
    // An exposed JS API does not prove an embedded host implements its speech
    // service. Keep this unverified path isolated pending native integration.
    if (globalThis.HUILAISHI_NATIVE_ANDROID) return "none";
  ...
  async function prepareChallenge(options = {}) {
    ...
    if (!lang) return base;
    if (globalThis.HUILAISHI_NATIVE_ANDROID && nativeVoiceBridge()) return { ...base, ready:true, status:"ready", mode:options.allowNetwork?"native-network":"native-device", microphone:"native-permission-on-start" };
    ...
    if (mode.startsWith("native-")) return { ...base, ready:true, status:"ready", mode, microphone:"native-permission-on-start" };
  ...
  async function recognizeTarget(options = {}) {
    cancelChallenge();
    const requestGeneration = challengeGeneration;
    ...
    if (globalThis.HUILAISHI_NATIVE_ANDROID) return nativeVoiceBridge() ? recognizeNative(options,target,lang,threshold,maxMs) : empty("none");
    ...
    if (requestGeneration !== challengeGeneration) return empty("cancelled");
    return new Promise(resolve => {
      let settled = false;
      let timer = 0;
