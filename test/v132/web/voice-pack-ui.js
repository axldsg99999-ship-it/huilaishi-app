(() => {
  ...
  const ui = () => isThaiUi() ? {
    ...
    sheetKicker: "LEARNING · OFFLINE AUDIO", sheetTitle: "เลือกชุดเสียงตามหัวข้อ", close: "ปิดชุดเสียง",
    ...
    wifi: "แต่ละชุดครอบคลุมการ์ดเดิม 500 ใบและตัวอย่าง คำเพิ่มใหม่ยังไม่มีเสียงบันทึกออฟไลน์ แนะนำให้ดาวน์โหลดผ่าน Wi-Fi",
    installed: "ติดตั้งแล้ว", install: "ดาวน์โหลด", delete: "ลบ", cancel: "ยกเลิก", planned: "กำลังจัดทำเสียง", partial: "ยังสร้างไม่ครบ", failed: "ดาวน์โหลดไม่สำเร็จ", checking: "กำลังตรวจสอบ…", unavailable: "รวมเสียงคำศัพท์ L1 ไว้ในแอป iOS แล้ว", offlineAction: "มี L1 แล้ว", loading: "กำลังอ่านชุดเสียง…",
    levelNames: ["เอาตัวรอด", "ชีวิตประจำวัน", "เที่ยวและเข้าสังคม", "งานและชีวิต", "แสดงความคิดเห็น", "ภาษาและวัฒนธรรม"],
    readySummary: count => `ติดตั้งแล้ว ${count}/6 ชุด`, noneSummary: "เลือกดาวน์โหลดเฉพาะหัวข้อที่เรียน", confirmDelete: "แตะอีกครั้งเพื่อลบ", network: "ต้องเชื่อมต่ออินเทอร์เน็ตเพื่อดาวน์โหลดครั้งแรก", quota: "พื้นที่ว่างอาจไม่พอสำหรับชุดนี้", done: "ติดตั้งชุดเสียงแล้ว", removed: "ลบชุดเสียงแล้ว"
  } : {
    title: "学习示范声包", summary: "标准语速与慢速词汇示范 · 可按主题离线安装", action: "管理",
    sheetKicker: "LEARNING · OFFLINE AUDIO", sheetTitle: "选择学习示范声包", close: "关闭声音包",
    privacy: "仅下载原生音高的合成学习音，不是 S1 角色萌音；自动校验不能替代母语教师终审，也不包含你的录音。",
    wifi: "每组覆盖原有 500 张词卡及例句；本次新增词的离线录音尚未制作。下载时建议使用 Wi‑Fi。",
    installed: "已安装", install: "下载", delete: "删除", cancel: "取消", planned: "音频制作中", partial: "尚未完整生成", failed: "下载失败", checking: "正在校验…", unavailable: "L1 词头示范音已随 iOS 应用内置", offlineAction: "L1 已内置", loading: "正在读取声音包…",
    levelNames: ["生存开口", "日常基础", "旅行社交", "工作生活", "观点表达", "文化进阶"],
    readySummary: count => `已安装 ${count}/6 组`, noneSummary: "只装正在学习的主题，节省手机空间", confirmDelete: "再点一次确认删除", network: "首次下载需要联网", quota: "手机剩余空间可能不足", done: "声音包安装完成", removed: "声音包已删除"
  };
  const isStandaloneBuild = () => Boolean(window.HUILAISHI_NATIVE_IOS) || Boolean(window.SINGLE_FILE_BUILD)
  ...
  function inject() {
    const anchor = $("#preview-alai-voice");
  ...
  function render() {
    ...
    list.innerHTML = rows.map(item => {
      ...
      return `<article class="voice-pack-row ${classes}" data-pack-row="${escapeHtml(item.id)}">
        <span class="voice-pack-level" aria-hidden="true">♪</span>
