...
(function (root) {
  ...
  function stop(){root.XULONG_VOICE_STUDIO?.close();clearTimeout(wardrobeTimer);epoch++;clearTimeout(audioTimer);root.HUILAISHI_SPEECH?.stop();root.XULONG_MUSIC?.release('world');drawing=false;pointId=null;busy=false;const button=q('#wj-listen');if(button?.disabled&&!settled){button.disabled=false;status(t('播放已暂停，可以重新听。','หยุดเสียงแล้ว แตะฟังใหม่ได้'));}}
  ...
  function world(){
    const s=M.stats(state),next=M.next(state),scene=M.scenes[dir][next?.scene??2],balance=wardrobe()?.read().balance||0;
    return `<div class="wj-world wj-world-play">
      <section class="wj-cover wj-play-cover" aria-label="${esc(scene.name)}">${picture(scene)}
        <div class="wj-play-title"><span class="wj-kicker">${t('泰语世界 / 小艾','โลกภาษาจีน / CHANINDA')}</span><h1 id="wj-focus" tabindex="-1">${esc(scene.name)}</h1><p>${t('校园声斗赛 · 听音与认字','ศึกเสียง · ฟังและอ่าน')}</p></div>
        <button type="button" class="wj-hero-greet" data-wj="greet" aria-label="${t('和小艾打招呼','ทักทาย CHANINDA')}" data-speech-skip><span>${t('点我打个招呼','แตะเพื่อทักทาย')}</span></button>
        <p class="wj-hero-message" role="status" hidden></p>
        <div class="wj-play-start">${b('campus',`<span><small>${t('准备好，就出发','พร้อมแล้ว ออกเดินทาง')}</small><b>${t('进入声斗赛','เข้าสู่ศึกเสียง')}</b></span><svg aria-hidden="true" viewBox="0 0 24 24"><path d="m9 5 7 7-7 7" fill="none" stroke="currentColor" stroke-width="2"/></svg>`,'','wj-primary')}</div>
      </section>
      <nav class="wj-practice-dock" aria-label="${t('练习方式','วิธีฝึก')}">
        ${b('practice',`${icon('volume')}<b>${t('听一听','ฝึกฟัง')}</b><small>${t('声音找物品','ฟังแล้วเลือกของ')}</small>`,'data-task="first-sound"','wj-practice-link')}
        ${b('speak',`<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="9" y="2" width="6" height="12" rx="3" fill="none"/><path d="M5 10v2a7 7 0 0 0 14 0v-2M12 19v3M8 22h8" fill="none" stroke="currentColor"/></svg><b>${t('开口练','ฝึกพูด')}</b><small>${t('麦克风 · 跟读','ไมค์ · พูดตาม')}</small>`,'','wj-practice-link wj-practice-speak')}
        ${b('practice',`${icon('book')}<b>${t('认字词','ฝึกอ่าน')}</b><small>${t('看字选意思','อ่านแล้วเลือก')}</small>`,'data-task="market-label"','wj-practice-link')}
      </nav>
      <section class="wj-story-strip"><div><small>${t('剧情手记','บันทึกเรื่องราว')} · ${s.steps}/${s.total}</small><h2>${next?copy(next.title):t('第一封回信已寄出','ส่งจดหมายฉบับแรกแล้ว')}</h2><div class="wj-story-dots" aria-hidden="true">${Array.from({length:s.total},(_,i)=>`<i class="${i<s.steps?'is-done':''}"></i>`).join('')}</div></div>${b(next?'start':'notebook',next?t('继续故事','เดินทางต่อ'):t('查看手记','ดูบันทึก'),'','wj-quiet')}</section>
      <div class="wj-home-utilities">${b('wardrobe',`${icon('cards')}<span>${t('衣橱','ตู้เสื้อผ้า')}<small>${balance} ${t('点可用','แต้ม')}</small></span>`)}${b('map',`${icon('location')}<span>${t('世界地图','แผนที่โลก')}</span>`)}</div>
    </div>`;
  }
  function notebook(){
  ...
  async function action(button){
    ...
    if(a==='speak'){root.XULONG_VOICE_STUDIO?.open({direction:dir,word:M.words[M.next(state)?.word]||M.words.water,returnFocus:button});return;}
    if(a==='greet'){
      clearTimeout(wardrobeTimer);const img=q('.wj-play-cover .wj-actor'),message=q('.wj-hero-message'),token=epoch;
      if(img)img.src=asset(hero('victory'));if(message){message.hidden=false;message.textContent=t('先听一遍，再开口。准备好就出发！','ฟังก่อน แล้วลองพูด พร้อมแล้วไปกันเลย!');}
      wardrobeTimer=setTimeout(()=>{if(token!==epoch)return;if(img?.isConnected)img.src=asset(hero());if(message?.isConnected)message.hidden=true;},2300);return;
    }
