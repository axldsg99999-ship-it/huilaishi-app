(function () {
  ...
  function showStatus(text, lang, voice, state = "speaking", reason = "audio-unavailable", options = {}) {
    ...
    // A diegetic cue already owns the visual status in combat. In listening
    // challenges never put the transcript into an overlay or accessible label.
    node.classList.toggle("is-visible", options.statusPresentation !== "scene");
    node.querySelector("b").textContent = options.hideTranscript
      ? (thaiUi ? "เสียงโจทย์" : "题目声音") : cleanText(text, 46);
  ...
  function clearActive() {
    window.XULONG_MUSIC?.release('speech');
  ...
  function reportPlaybackError(text, lang, options, runId, track = "standard", reason = "audio-unavailable") {
    ...
    showStatus(text, lang, voice, "error", reason, options);
  ...
  function playBundled(text, lang, asset, options, runId) {
    ...
    audio.addEventListener("playing", () => {
      ...
      showStatus(text, lang, voice, "speaking", "", options);
      try { options.onStart?.({lang,track,duration:audio.duration}); } catch (_) {}
    }, { once: true });
  ...
  function playDeviceSpeech(text, options, runId, fallbackLang) {
    ...
    utterance.onstart = () => {
      if (runId !== sequenceId) return;
      showStatus(text, lang, deviceMeta, "speaking", "", options);
      try { options.onStart?.({lang,track:options.track||"standard"}); } catch (_) {}
    };
    utterance.onend = () => {
      if (runId !== sequenceId) return;
      clearActive();
  ...
  function speak(value, options = {}) {
    ...
    window.XULONG_MUSIC?.hold('speech');
    if ("speechSynthesis" in window) window.speechSynthesis.cancel();
    if (activeAudio) {
      activeAudio.pause();
      activeAudio.currentTime = 0;
  ...
  function speakSequence(parts, options = {}) {
    ...
    const playNext = () => {
      ...
      if (result.voiceMissing) {
        clearActive();
        showStatus(item.text, result.lang, null, "error", "audio-unavailable", { ...options, ...item });
        return;
      }
      activeUtterance = result.utterance;
      result.utterance.onstart = () => { if (runId === sequenceId) showStatus(item.text, result.lang, result.voice, "speaking", "", { ...options, ...item }); };
      ...
      result.utterance.onerror = () => {
        if (runId !== sequenceId) return;
        clearActive();
        showStatus(item.text, result.lang, result.voice, "error", "audio-unavailable", { ...options, ...item });
      };
