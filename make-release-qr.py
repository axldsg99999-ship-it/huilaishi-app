...
ANDROID_URL = "https://github.com/axldsg99999-ship-it/huilaishi-app/releases/download/v12.4.0-samsung.1/huilaishi-samsung-12.4.0-r1-release.apk"
...
def make_board(apple: Image.Image, android: Image.Image) -> None:
    ...
    draw.text((82, 70), "会来事 12.4 · 手机安装", fill=PAPER, font=font(42, True))
    draw.text((82, 130), "中国人学泰语 · 泰国人学中文 · 双向互动", fill=MUTED, font=font(22))

    cards = [
        (70, "APPLE · 现在可用", "Safari 安装 PWA", "分享 → 添加到主屏幕", apple, TEAL),
        (650, "SAMSUNG · 12.4-R1", "下载签名 APK", "支持从 12.3 覆盖升级", android, LIME),
    ]
    ...
    board.save(OUTPUT / "会来事12.4-双平台安装二维码.png")


def main() -> None:
    apple = save_qr(APPLE_URL, "会来事12.4-苹果安装二维码.png")
    android = save_qr(ANDROID_URL, "会来事12.4-三星APK二维码.png")
