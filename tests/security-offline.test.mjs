...
test("failed shell staging rejects install and preserves the verified legacy fallback", async () => {
  ...
  assert.equal((await harness.cacheStorage.keys()).includes("huilaishi-offline-v58"), false);
  assert.equal((await harness.cacheStorage.keys()).includes("huilaishi-offline-v58-installing"), false);
...
test("successful shell staging commits readiness before taking control", async () => {
  ...
  const current = await harness.cacheStorage.open("huilaishi-offline-v58");
  const marker = await current.match(`${SCOPE}__huilaishi_base_ready_v58__`);
...
test("CLEAR_CORE_AUDIO pauses the job and removes current and legacy audio copies", async () => {
  const harness = serviceWorkerHarness();
  const current = await harness.cacheStorage.open("huilaishi-offline-v58");
  await current.put(`${SCOPE}__huilaishi_base_ready_v58__`, new Response(JSON.stringify({ phase: "base-ready" })));
  const legacy = await seedReadyLegacyShell(harness.cacheStorage);
  const runtime = await harness.cacheStorage.open("huilaishi-runtime-v58");
